...
- [~] **I2. i18n 产品文案中英收口 — Frontend**（**进行中**：已启用 `gen-l10n` + 首页语言卡片；**绝大部分界面仍为硬编码**，需按模块迁移到 ARB）
  ...
  - [ ] I2.2 覆盖项目列表界面的中英文文案
  - [ ] I2.3 覆盖工作台界面的中英文文案
  - [~] I2.4 覆盖设置界面的中英文文案（除语言卡片外）— **部分**：**`_PlatformConfigSection`** 主文案、本机偏好说明、操作按钮、七项功能开关、Plan/Workspace/User 覆盖说明与 SnackBar / 未登录提示已 **`platformConfig*`** ARB；**`build_sections_product`** 内合规跳转 SnackBar、产品导航标签、禁用原因等仍硬编码
