...
### 需求 15：自动化记忆深化 — 角色级风格记忆与预算分层

**用户故事：** 作为平台运营者，我希望系统在视频提示词生成时，不只是“有记忆”，而是能按角色、场景风险和上下文预算智能选择记忆，这样既能保住人物风格和情绪，又不会把 token 无意义烧满。

#### 验收标准

1. THE System SHALL 支持在项目级与剧本级全局风格记忆之外，引入角色级视频风格记忆，用于记录特定角色的表演习惯、说话质感、镜头偏好和高风险禁忌。
2. WHEN 当前分镜能识别出主要角色主体时，THE System SHALL 优先检索并注入与该角色主体匹配的角色级风格记忆，而不是只注入项目级通用风格摘要。
3. WHEN 当前镜头为低风险、设定稳定、参考信息完整的场景时，THE System SHALL 使用 `lean` 记忆预算层级，只注入最少但必要的风格约束。
4. WHEN 当前镜头缺少参考图、情绪脆弱、连续性风险高或人物锚点不足时，THE System SHALL 自动提升到 `expanded` 记忆预算层级，允许注入更多角色与连续性信息。
5. THE System SHALL 记录每次视频提示词构建所使用的 `memory_budget_tier` 诊断信息，便于后续分析“哪些高质量结果确实需要更多记忆”。
6. IF 当前镜头无台词、低风险且角色身份稳定，THEN THE System SHALL 避免注入冗长的声音或表演记忆，优先保留视觉与连续性约束。
7. THE System SHALL 支持在多角色场景中优先选择主主体角色记忆；若多个角色均命中，须按当前镜头主叙事焦点排序，而不是简单拼接全部角色记忆。

---

### 需求 16：提示词内控深化 — 观察笔记、负向约束与低信号淘汰

**用户故事：** 作为平台运营者，我希望系统在吸收历史失败经验时，只保留真正有用、与当前镜头相关、且不与风格约束冲突的观察笔记，避免把大量模糊、重复、低信号的“失败经验”塞进提示词。

#### 验收标准

1. WHEN 系统从历史失败记录、返工记录或质量评审中提取观察笔记时，THE System SHALL 对候选观察笔记执行相关性筛选，只保留与当前镜头主体、台词风险、动作风险、光影风险或连续性风险相关的内容。
2. THE System SHALL 自动淘汰泛泛而谈、没有修复指向性的低信号观察笔记，例如只有“注意情绪”“避免重复问题”这类不具备可操作约束的信息。
3. WHEN 观察笔记与当前画风、镜头结构或角色风格记忆发生明显冲突时，THE System SHALL 过滤掉冲突片段，必要时只保留其中不冲突的半句约束。
4. WHEN 当前镜头是简短填充对白、无台词镜头或低风险静态镜头时，THE System SHALL 跳过与口型、发声或复杂表演无关的观察笔记，避免无效注入。
5. WHEN 系统自动把观察笔记转化为负向约束时，THE System SHALL 记录该负向约束的来源，如 `pending_observation_note`、`rejected_memory` 等诊断标签。
6. THE System SHALL 支持在高压缩上下文模式下优先保留“具体、短、与当前风险强相关”的观察笔记，而不是保留泛化摘要。
7. THE System SHALL 在提示词构建诊断信息中暴露 `observation_note_chars`、`negative_budget_tier` 和 `auto_negative_source`，用于后续排查为何某次结果质量好或差。

---

### 需求 17：运营与成本分析 — token 用量和质量结果关联

**用户故事：** 作为平台运营者，我希望不只是看到用了多少 token，还能知道这些 token 花得值不值，哪些调用类型真正带来了质量提升，哪些只是烧钱。

#### 验收标准

1. THE System SHALL 对每次大模型调用记录 `prompt_tokens`、`completion_tokens`、`total_tokens`、`prompt_chars`、`model_name`、`call_type`、`duration_ms`、`success` 等基础用量信息。
2. THE System SHALL 支持把同一生成任务的质量评审结果关联回对应的大模型调用记录，至少关联 `quality_review_id` 和 `overall_score`。
3. WHEN 平台运营者查看成本分析时，THE System SHALL 支持按阶段、模型、调用类型、项目、用户维度查看 token 消耗与平均质量得分的关系。
4. THE System SHALL 支持识别“高 token 低质量”的调用模式，至少能够按 `call_type` 和 `model_name` 聚合输出异常区间或低 ROI 区间。
5. WHEN 某类自动化记忆注入策略或返工策略显著增加 token 消耗时，THE System SHALL 支持对比变更前后的质量提升是否足以覆盖新增成本。
6. THE System SHALL 保持这类分析的隔离性，禁止使用跨用户模糊匹配去关联质量和用量数据；优先使用明确的 `job_id` 或等价显式关联字段。
7. THE System SHALL 为后续引入“按项目记忆 ROI 排名”“按模型 bad case 成本排名”“按阶段返工收益排名”预留数据基础。

---

### 需求 18：任务中心完善 — 失败重试、配额反馈与实时状态可见

**用户故事：** 作为平台用户或运营者，我希望能在任务中心清楚看到生成任务的状态变化、失败原因、可重试性和配额反馈，这样我不会在失败后无从判断下一步怎么做。

#### 验收标准

1. THE System SHALL 通过 WebSocket `generation.job.updated` 或等价实时机制，把任务从 `queued`、`running`、`succeeded`、`failed`、`cancelled` 到 `retried` 的状态变化推送给当前用户。
2. THE TaskCenter 或等价前端界面 SHALL 在任务列表中展示任务类型、状态、更新时间、失败原因摘要和当前是否可取消/可重试。
3. WHEN 任务状态为 `failed` 时，THE TaskCenter SHALL 提供显式重试入口；调用 `POST /api/v1/jobs/{id}/retry` 成功后，界面状态应立即反映为重新排队。
4. WHEN 用户触发取消操作时，THE TaskCenter SHALL 对 `queued` 和 `running` 任务展示取消反馈，并在状态变更后实时刷新。
5. IF 后端返回 `quota_exceeded` 或 HTTP `429`，THEN THE TaskCenter SHALL 展示可理解的配额提示，并利用 `Retry-After` 或 `retry_after_ms` 给出明确的等待时间反馈。
6. THE System SHALL 区分“可以重试的失败”和“需要先修上游数据/配置后再重试的失败”，避免用户机械地点重试。
7. THE System SHALL 支持在任务详情中展示最近一次错误上下文、重试时间和最新状态更新时间，帮助用户和运营者快速判断任务是否仍值得继续。
...
| 新质量优化 | 人物锚点、情绪起伏、反 AI 痕迹检查、真实自然视频表现、观察笔记去低信号与去冲突 |
| 新成本优化 | 最小上下文读取、记忆注入计划、项目级记忆预算、记忆压缩与淘汰、token 与质量 ROI 关联 |
| 新平台能力 | 类 Codex 自动化记忆的项目化引入、角色级风格记忆、任务中心重试与配额反馈 |
...
| 需求 15（角色级记忆与预算分层） | 视频提示词记忆选择器、角色级风格记忆、`memory_budget_tier` 诊断 | ❌ 已有实现线索，规格待补齐后系统化实现 |
| 需求 16（观察笔记与负向约束治理） | 观察笔记筛选、冲突过滤、低信号淘汰、`auto_negative_source` 诊断 | ❌ 已有实现线索，规格待补齐后系统化实现 |
| 需求 17（token-质量关联分析） | `app_llm_usage_log`、质量评审关联、ROI 统计接口/视图 | ❌ 底层埋点已有，产品化分析待完善 |
| 需求 18（任务中心完善） | `generation.job.updated`、`POST /api/v1/jobs/{id}/retry`、`retry_after_ms` / `Retry-After` 反馈 | ❌ 接口/推送已有基础，产品体验待补齐 |
