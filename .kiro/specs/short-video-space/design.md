...
本 spec 将 `space/short-video` 中的产品结论与实施波浪落为可执行设计：在复用 Openflow 现有生产域的前提下，增加项目级短视频配置、分镜 readiness 与候选确认、成片装配与导出校验、独立发布域与多平台适配，并明确与开源参考项目的边界。
...
短视频目标（画幅、市场、平台、时长策略、声线、字幕、BGM、模式等）写入项目级元数据，脚本、分镜、导出、发布统一读取，避免「单次任务参数」与 Space 展示不一致（对齐 MoneyPrinterTurbo 的集中参数价值，但数据模型贴合 Openflow）。
...
先统一读取已选镜头媒体、旁白、字幕、BGM 完成 **可运营的 rough cut** 与导出前检查；**非电影级**时间线编辑在 γ 波次完整交付（见 `requirements` 核心约束：禁止剪辑凑合交付）。好莱坞级无限轨 NLE 不在首期范围。
...
**impl Wave 1 / MP-W2** 向的项目配置优先项目扩展字段；**impl Wave 4+** 发布域引入独立表 `publish_*`；分镜与候选若现有模型不足再增量迁移，避免过早拆表阻塞竖切。
