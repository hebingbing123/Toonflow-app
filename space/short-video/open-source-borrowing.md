...
- Toonflow 不该重做成轻量单体生成器，而应把这些长处迁入现有 Rust + Flutter + PG 的工作流体系
...
- 直接照搬其前后端技术栈没有必要
- 若没有结合 Toonflow 现有项目/剧本/分镜/资产模型，孤立搬 readiness 也会失真
...
Toonflow 最合适的组合方式不是二选一，而是：

- 用 `MoneyPrinterTurbo` 的入口感
- 接 `Jellyfish` 的准备态和任务闭环
- 再叠加 Toonflow 自己的自动发布能力
