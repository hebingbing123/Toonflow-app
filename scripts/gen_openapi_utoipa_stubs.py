#!/usr/bin/env python3
"""Generate utoipa path stubs from an OpenAPI YAML `paths:` document (partitioned OpenApi structs + merge helper).
...
SKIP_OPERATION_IDS = frozenset(
    {
        ...
        "listHarnessToolsV1",
        "validateHarnessUserWasmV1",
        "persistHarnessUserWasmV1",
        "listHarnessUserWasmV1",
        "websocketUpgrade",
        "usageSummaryV1",
        "cancelJobV1",
        "createJobV1",
...
def snake_to_pascal(s: str) -> str:
    return "".join(part.capitalize() for part in s.split("_") if part) or "Api"
...
def schema_to_rust_content(schema: Any) -> str:
    ...
    if isinstance(schema, dict) and isinstance(schema.get("$ref"), str):
        ...
        if ref.startswith(REF_PREFIX):
            ...
            return f'ref("{name}")'
        # Stub fixtures may already flatten refs to bare component names.
        if "/" not in ref and ref and not ref.startswith("#"):
            return f'ref("{ref}")'
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Regenerate openapi_spec/generated partition stubs from an OpenAPI YAML `paths:` section.",
    )
    ap.add_argument(
        "openapi_yaml",
        nargs="?",
    ...
    for path, method, op in ops:
        tags = op.get("tags") or []
        tag0 = tags[0] if tags and isinstance(tags[0], str) else "api"
        grouped[tag_slug(tag0)].append((path, method, op))

    modules: list[tuple[str, str, list[tuple[str, str, dict[str, Any]]]]] = []
    ...
    for stale in OUT_DIR.glob("*.rs"):
        if stale.name != "mod.rs":
            stale.unlink()

    for module_name, struct_name, part in modules:
        lines: list[str] = []
        lines.append("//! AUTO-GENERATED by `scripts/gen_openapi_utoipa_stubs.py`. Do not edit.")
        lines.append("")
        lines.append("mod stubs {")
        for idx, (path, method, op) in enumerate(part):
        ...
        lines.append("}")
        path_list = ",\n    ".join(
            f"stubs::{rust_ident(op['operationId'])}" for _, _, op in part
        )
        ...
        (OUT_DIR / f"{module_name}.rs").write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    for module_name, _struct_name, _part in modules:
    ...
    if not modules:
        ...
        first_module, first_struct, _ = modules[0]
        mod_lines.append(f"    let mut doc = {first_module}::{first_struct}::openapi();")
        for module_name, struct_name, _part in modules[1:]:
            mod_lines.append(f"    doc.merge({module_name}::{struct_name}::openapi());")
        mod_lines.append("    doc")
    mod_lines.append("}")

    (OUT_DIR / "mod.rs").write_text("\n".join(mod_lines) + "\n", encoding="utf-8")

    print(f"Wrote {len(modules)} generated file(s) with {len(ops)} stubs under {OUT_DIR}")
