...
pub fn harness_user_wasm_persist_http(user_id: Uuid, size_bytes: usize) {
    tracing::debug!(
        %user_id,
        size_bytes,
        "harness.http.user_wasm_persist"
    );
}

pub fn harness_user_wasm_list_http(user_id: Uuid, returned: usize) {
    tracing::debug!(
        %user_id,
        returned,
        "harness.http.user_wasm_list"
    );
}
