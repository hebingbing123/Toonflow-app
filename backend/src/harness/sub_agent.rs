...
fn sub_agent_spec(tool_name: &str) -> Option<SubAgentSpec> {
    match tool_name {
        ...
        "run_sub_agent_production_supervision" => Some(SubAgentSpec {
            ...
            execution_hint: Some(
                "审核必须基于工具实读的数据，优先读取 storyboardTable/script/assets 的必要字段或窗口；若问题只涉及部分资产，下一步给出 check_assets 时要把真实 assetIds 回填到 reviewSummary；若只涉及部分缺帧或待核对镜头，下一步给出 check_storyboard 或 generate_storyboard 时要把真实 storyboardIds 回填到 reviewSummary，避免无差别全量读取。",
            ),
        }),
        _ => None,
    }
