...
// =============================================================================
// Workbench Remaining Endpoints (Wave E)
// =============================================================================

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
struct GenerateVideoPromptBody {
    project_id: i32,
    script_id: i32,
    #[serde(default)]
    #[allow(dead_code)]
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct GenerateVideoPromptResponse {
    prompt: String,
    model: String,
    duration: i32,
}

async fn post_workbench_generate_video_prompt(
    State(state): State<AppState>,
    headers: HeaderMap,
    ...
    // Generate a video prompt based on description or default
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct GetGenerateDataResponse {
    project_id: i32,
    script_id: i32,
...
async fn post_workbench_get_generate_data(
    State(state): State<AppState>,
    headers: HeaderMap,
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct VideoModelDetailResponse {
    model_id: String,
    model_name: String,
    provider: String,
    max_duration: i32,
...
async fn post_workbench_get_video_model_detail(
    State(state): State<AppState>,
    headers: HeaderMap,
) -> Result<JsonResponse<VideoModelDetailResponse>, ApiError> {
    let _uid = require_user_uuid(&state, &headers)?;

    // Return the default video model details
...
pub fn router() -> Router<AppState> {
    let mut r = Router::new()
        ...
        .route(
            ...
            post(post_workbench_generate_video_prompt),
        )
        .route(
            "/api/v1/production/workbench/get-generate-data",
            post(post_workbench_get_generate_data),
        )
        .route(
