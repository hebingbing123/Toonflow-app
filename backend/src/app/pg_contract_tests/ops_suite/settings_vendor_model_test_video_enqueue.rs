...
async fn settings_vendor_model_test_video_kind_enqueue() {
    ...
    let body = r#"{"modelName":"doubao-seedance-1-0-pro","type":"video","id":"20"}"#;
    let res = app
        .oneshot(
            Request::builder()
                .method(Method::POST)
    ...
    assert_eq!(status, StatusCode::OK, "model-test video body={job}");
    assert_eq!(
        job["kind"].as_str(),
    ...
    let payload = job
        .get("payload")
        .or_else(|| job.get("Payload"))
        .unwrap_or(&job);
    assert_eq!(
        payload.get("kind").and_then(|k| k.as_str()),
        Some("video"),
        "job payload should record video model-test kind: {job}"
    );
}
