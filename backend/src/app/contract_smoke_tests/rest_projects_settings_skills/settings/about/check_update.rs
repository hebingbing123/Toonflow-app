...
async fn settings_about_check_update_stub_ok_with_jwt() {
    ...
    std::env::remove_var("OPENFLOW_UPDATE_LATEST_VERSION");
    std::env::remove_var("OPENFLOW_UPDATE_TIME");
    std::env::remove_var("OPENFLOW_UPDATE_OPENFLOW_URL");
    std::env::remove_var("OPENFLOW_UPDATE_GITHUB_URL");
    std::env::remove_var("OPENFLOW_UPDATE_GITEE_URL");
    std::env::remove_var("OPENFLOW_UPDATE_ATOMGIT_URL");
...
async fn settings_about_check_update_uses_env_manifest_with_jwt() {
    let _guard = env_test_lock().await;
    std::env::set_var("OPENFLOW_UPDATE_LATEST_VERSION", "0.1.1");
    std::env::set_var("OPENFLOW_UPDATE_TIME", "2026-04-08T08:30:00Z");
    std::env::set_var(
        "OPENFLOW_UPDATE_GITHUB_URL",
        "https://example.com/openflow-0.1.1.zip",
    );
    let token = test_jwt(Uuid::nil());
    let (status, v) = post_json_bearer(
        "/api/v1/settings/about/check-update",
    ...
    assert_eq!(v["url"], "https://example.com/openflow-0.1.1.zip");
    std::env::remove_var("OPENFLOW_UPDATE_LATEST_VERSION");
    std::env::remove_var("OPENFLOW_UPDATE_TIME");
    std::env::remove_var("OPENFLOW_UPDATE_OPENFLOW_URL");
    std::env::remove_var("OPENFLOW_UPDATE_GITHUB_URL");
