...
#[tokio::test]
async fn scripts_batch_add_unauthorized_without_bearer() {
    let (status, v) = post_json(
        "/api/v1/scripts/batch-add",
        r#"{"projectId":1,"data":[{"scriptName":"a","scriptData":"b"}]}"#,
    )
    .await;
    assert_eq!(status, StatusCode::UNAUTHORIZED);
    assert_eq!(v["code"], "unauthorized");
...
#[tokio::test]
async fn scripts_batch_add_requires_database_with_jwt() {
    let token = test_jwt(Uuid::nil());
    let (status, v) = post_json_bearer(
        "/api/v1/scripts/batch-add",
        &token,
