...
mod pg_contract_tests {
    ...
    async fn asset_image_file_local_storage_roundtrip() {
        ...
        let res = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .header(header::CONTENT_TYPE, "application/json")
                    .extension(ConnectInfo(test_addr()))
                    .body(Body::from(r#"{"name":"test_role","type":"role"}"#))
                    .unwrap(),
            )
            .await
            .unwrap();
        ...
        let asset_id = asset["legacy_id"].as_i64().expect("legacy_id") as i32;

        // Create image with local storage metadata
        let res = app
            .clone()
            .oneshot(
                Request::builder()
                    .method(Method::POST)
                    .uri(format!(
                        "/api/v1/projects/legacy/{project_id}/assets/{asset_id}/images"
                    ))
                    .header(header::AUTHORIZATION, format!("Bearer {token}"))
                    .header(header::CONTENT_TYPE, "application/json")
                    .extension(ConnectInfo(test_addr()))
                    .body(Body::from(
                        r#"{"file_path":"/api/v1/projects/legacy/0/assets/0/images/00000000-0000-0000-0000-000000000000/file","state":"已完成","sort_index":0,"metadata":{"storage":"local"}}"#,
                    ))
                    .unwrap(),
            )
            .await
            .unwrap();
        let (status, img) = read_json_response(res).await;
        assert_eq!(status, StatusCode::CREATED, "img={img}");
        let img_uuid = img["id"].as_str().expect("image id");

        // Write test PNG file to temp directory
        ...
        let res = app
            ...
            .oneshot(
                Request::builder()
                    .method(Method::PATCH)
                    .uri(format!(
                        "/api/v1/projects/legacy/{project_id}/assets/{asset_id}/images/{img_uuid}"
                    ))
                    .header(header::AUTHORIZATION, format!("Bearer {token}"))
                    .header(header::CONTENT_TYPE, "application/json")
                    .extension(ConnectInfo(test_addr()))
                    .body(Body::from(r#"{"file_path":"/api/v1/projects/legacy/0/assets/0/images/00000000-0000-0000-0000-000000000000/file"}"#))
                    .unwrap(),
            )
            .await
            .unwrap();
        let (status, _) = read_json_response(res).await;
        assert_eq!(status, StatusCode::OK, "patch image");

        // Direct DB update to set metadata.storage = "local" since API doesn't expose metadata patch
        let _ = sqlx::query(
            "UPDATE app_asset_image SET metadata = jsonb_build_object('storage', 'local') WHERE id = $1"
        )
        .bind(Uuid::parse_str(img_uuid).unwrap())
        .execute(&pool)
        .await
        .expect("set local storage metadata");
        ...
        let res = app
            ...
            .oneshot(
                Request::builder()
                    .uri(format!(
                        "/api/v1/projects/legacy/{project_id}/assets/{asset_id}/images/{img_uuid}/file"
                    ))
                    .header(header::AUTHORIZATION, format!("Bearer {token}"))
                    .extension(ConnectInfo(test_addr()))
                    .body(Body::empty())
        ...
        let _ = sqlx::query("DELETE FROM public.app_asset_image WHERE asset_id IN (SELECT id FROM public.app_asset WHERE project_id IN (SELECT id FROM public.app_project WHERE legacy_id = $1))")
            .bind(project_id)
            .execute(&pool)
            .await;
        let _ = sqlx::query("DELETE FROM public.app_asset WHERE project_id IN (SELECT id FROM public.app_project WHERE legacy_id = $1)")
            .bind(project_id)
            .execute(&pool)
            .await;
        let _ = sqlx::query("DELETE FROM public.app_project WHERE legacy_id = $1")
