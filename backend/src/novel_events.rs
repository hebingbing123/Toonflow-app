...
mod tests {
    ...
    fn create_novel_event_body_rejects_unknown_fields() {
        let err = serde_json::from_str::<CreateNovelEventBody>(
            r#"{"name":"Event","extra":1}"#,
        );
        assert!(err.is_err());
    }
    ...
    fn create_novel_event_body_accepts_minimal() {
        let b: CreateNovelEventBody =
            serde_json::from_str(r#"{"name":"Test Event"}"#).unwrap();
    ...
    fn create_novel_event_body_accepts_full() {
        let b: CreateNovelEventBody = serde_json::from_str(
            r#"{"name":"Event","detail":"Details","chapterIds":[1,2,3]}"#,
        )
        .unwrap();
    ...
    fn update_novel_event_body_rejects_unknown_fields() {
        let err = serde_json::from_str::<UpdateNovelEventBody>(
            r#"{"name":"New","extra":1}"#,
        );
        assert!(err.is_err());
    }
    ...
    fn update_novel_event_body_accepts_detail_clear() {
        let b: UpdateNovelEventBody =
            serde_json::from_str(r#"{"detail":null}"#).unwrap();
        assert_eq!(b.detail, Some(Value::Null));
    }

    #[test]
    fn update_novel_event_body_accepts_detail_string() {
        let b: UpdateNovelEventBody =
            serde_json::from_str(r#"{"detail":"New detail"}"#).unwrap();
    ...
    fn update_novel_event_body_accepts_chapter_ids() {
        let b: UpdateNovelEventBody =
            serde_json::from_str(r#"{"chapterIds":[10,20]}"#).unwrap();
        assert_eq!(b.chapter_ids, Some(vec![10, 20]));
    }

    #[test]
    fn batch_delete_events_body_rejects_unknown_fields() {
        let err = serde_json::from_str::<BatchDeleteEventsBody>(
            r#"{"ids":[1,2],"extra":1}"#,
        );
        assert!(err.is_err());
    }
    ...
    fn legacy_get_events_body_accepts_valid() {
        let b: LegacyGetEventsBody = serde_json::from_str(
    ...
    fn legacy_get_events_body_accepts_with_search() {
        let b: LegacyGetEventsBody = serde_json::from_str(
            r#"{"projectId":1,"page":1,"limit":20,"search":"test"}"#,
        )
        .unwrap();
    ...
    fn search_ilike_returns_pattern_for_valid() {
        assert_eq!(search_ilike(Some("test".to_string())), Some("%test%".to_string()));
        assert_eq!(search_ilike(Some("  test  ".to_string())), Some("%test%".to_string()));
    }
    ...
    fn batch_delete_response_serialize() {
        let resp = BatchDeleteEventsResponse { message: "删除成功" };
