...
const MAX_NOVEL_LIST_LIMIT: i64 = 200;
...
pub fn router() -> Router<AppState> {
    Router::new()
        .route(
            ...
            get(list_novels).post(create_novel),
        )
        .route(
            "/api/v1/projects/legacy/{project_legacy_id}/novels/{novel_legacy_id}",
            get(get_novel_by_legacy)
                .patch(patch_novel_by_legacy)
                .delete(delete_novel_by_legacy),
        )
}
...
async fn patch_novel_by_legacy(
    State(state): State<AppState>,
    Path((project_legacy_id, novel_legacy_id)): Path<(i32, i32)>,
    headers: HeaderMap,
...
async fn delete_novel_by_legacy(
    State(state): State<AppState>,
    Path((project_legacy_id, novel_legacy_id)): Path<(i32, i32)>,
    headers: HeaderMap,
...
mod tests {
    ...
    fn patch_novel_body_rejects_unknown_fields() {
        let err =
            serde_json::from_str::<PatchNovelBody>(r#"{"chapter":"a","extra":1}"#).unwrap_err();
        assert!(
            err.to_string().contains("unknown field")
                || err.to_string().contains("unknown variant"),
            "{err}"
