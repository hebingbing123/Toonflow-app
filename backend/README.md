...
**HTTP 接口文档（浏览器）**：`GET http://127.0.0.1:8666/api/v1/docs` — Swagger UI **Standalone** 布局（顶栏 + 下载等；加载同源的 **`/api/v1/openapi.yaml`**，内容编译期嵌入仓库根 **`docs/openapi.yaml`**）。给前端联调时直接发该 URL 即可。
