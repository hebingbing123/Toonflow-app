...
填充 `legacy_user_map` 后，在 Supabase SQL（**service_role**）执行 `SELECT * FROM public.promote_legacy_from_staging();` 写入 `app_project` / `app_script` / `app_storyboard` / **`app_novel`** / **`app_asset`** / **`app_script_asset`** / **`app_art_style`** / **`app_user_prompt`**（**`o_artStyle`**、**`o_prompt`**；**`owner_user_id`** 取映射表中 **`legacy_user_id` 最小** 的一行；返回值八列含 **`prompts_upserted`** 等）。详见 [`docs/migration/legacy-sqlite-to-supabase.md`](../docs/migration/legacy-sqlite-to-supabase.md)。
