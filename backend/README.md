...
**HTTP 接口文档（浏览器）**：`GET http://127.0.0.1:8666/api/v1/docs` — Swagger UI **Standalone** 布局；**默认折叠**所有 tag、顶部带 **Filter** 搜索；`openapi.yaml` 的 `info.description` 保持简短，详细叙事见 **`docs/plans/electron-node-parity.md`** / **`backend/README.md`**。给前端直接发该 URL 即可（改 spec 后需重新 `cargo run` 以嵌入最新 YAML）。
