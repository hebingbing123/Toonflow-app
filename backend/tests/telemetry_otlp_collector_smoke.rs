...
//!   RUST_LOG=info \
//!   cargo test -p openflow-server --test telemetry_otlp_collector_smoke otlp_export_reaches_collector -- --ignored --nocapture
//! ```
//!
...
async fn otlp_export_reaches_collector() {
    ...
    openflow_server::telemetry::init_otel_for_collector_smoke_test().expect("init otel");
