...
目标：{一句话目标}
上下文：{必要数据摘要}
画风技能包：{art_skills/xxx 路径，如 art_skills/realpeople_ancient_chinese}
故事风格技能包：{story_skills/xxx 路径，如 story_skills/Sweet_romance_novel}
要求：
1. {具体步骤1}
2. {具体步骤2}
约束：{特殊约束条件}
...
| 记忆检索无结果 | 向用户明确说明「无法确认当前进度，请告知当前所处阶段」，并列出以下六个阶段供用户选择：1. 阶段1：衍生资产分析 2. 阶段2：衍生资产生成（可选）3. 阶段3：导演规划 4. 阶段4：构建分镜表 5. 阶段5：分镜面板写入 6. 阶段6：分镜图生成；禁止基于猜测自行判断进度并派发任务，禁止开放式询问而不提供选项 |
