...
以下三条约束**必须在改编策略阶段明确声明**，而非推迟到剧本编写阶段才执行：

1. **单句台词不超过 20 字**：适配竖屏短视频观众阅读速度，超长台词须在改编策略中拆分或改写为动作描述
2. **开场 30 秒内必须有冲突或悬念建立点**：禁止以纯环境描写或人物介绍开场，改编策略须明确每集开场的冲突/悬念设计方向
3. **每集结尾必须有集末钩子**：最后 5-10 秒的台词或画面须设计为钩子，类型须多样化（悬念钩子、情感钩子、智识钩子、世界观钩子），禁止全部集数使用同一类型钩子
...
- **结构策略**：多线叙事的处理方式
- **风格标尺**：情绪/冲突/悬疑的度
- **载体约束**：短剧平台的特殊限制如何影响改编（必须声明竖屏短视频三条强制约束：单句台词≤20字、开场30秒内有冲突/悬念、每集结尾有集末钩子）
