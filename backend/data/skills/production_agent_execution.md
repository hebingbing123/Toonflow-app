...
写分镜与视频层描述时：情绪必须落到**可拍摄细节**（眼神、呼吸、肩手、步伐、声线），禁止单用「开心/难过/紧张」等空泛词；同一角色相邻分镜情绪强度须有起落，避免三连同档；含台词须给语速、停顿、气口线索，避免念稿感。与 `videoDesc` 情绪字段及导演规划中的情绪曲线保持一致。
...
5. 简单说明要增加的衍生资产内容以及信息。总共200字以内
6. 如不需要衍生资产，返回"不需要衍生资产"，流程结束
...
- **衍生资产去重规则**：
  - 在调用 `add_deriveAsset` 前，必须检查候选衍生资产与已存在衍生资产的相似度
  - 若候选衍生资产的名称或描述与已存在衍生资产相似度超过 80%（如"染血版"与"血迹版"、"夜景"与"夜晚版"），禁止创建新条目，应复用已有衍生资产
  - 相似度判断标准：名称核心词相同、或描述中的视觉差异描述高度重叠（超过 80% 的关键词相同）
  - 若不确定是否重复，优先复用已有衍生资产，而非创建新条目
- 每个资产 1~5 个衍生，宁缺勿滥
...
**表演锚点卡维护**：在开始构建分镜表前，为每个主要角色建立「表演锚点卡」，记录：
- 角色名
- 默认画面位置（左/中/右）
- 默认朝向（面朝左/面朝右）
- 情绪基准状态
...
每写一行分镜前，对照表演锚点卡校验当前分镜中该角色的位置、朝向是否与锚点一致；若有变化，须在 `action` 字段中标注位置/朝向变化的动作衔接。
...
完成全部分镜表后，执行以下 8 项反穿帮检查：

1. **动作连续性**：相邻分镜间角色位置、动作进度、朝向是否物理逻辑一致；`action` 字段是否标注了与上一镜的衔接关系
2. **景别递进**：是否出现连续 3 条以上使用完全相同景别的分镜（违反景别递进法则）
3. **视轴守恒**：对话/对峙场景中角色画面位置是否全程固定同侧（180度线原则）
4. **朝向空间逻辑**：对话双方是否面朝彼此，操作物品是否面朝物品
5. **定场镜头数量**：每个新场景的定场镜头是否不超过 2 个
6. **无台词镜头时长**：无台词镜头 `duration` 是否不超过 6 秒（一镜到底上限 12 秒）
7. **画质降级词**：所有分镜的 `lighting` 和 `description` 字段是否不含画质降级词（`film grain`、`imperfect focus`、`柔焦`、`朦胧感`等）
8. **台词-时长匹配**：含台词分镜的 `duration` 是否满足 `ceil(台词字数/语速)+1` 的最低时长要求
...
| 操作 | 调用 |
|------|------|
| 读取剧本 | `get_flowData({ key: "script", lineStart, lineEnd, maxChars })` |
...
1. 先读取 `storyboardTable` 当前最小窗口：若上游审核或派发指令已经给了明确 `storyboardIds`，优先调用 `get_flowData({ key: "storyboardTable", ids: [真实分镜ID列表], fields: [...] })`；只有未给出精确镜头时，才用 `get_flowData({ key: "storyboardTable", rowStart: 1, rowCount: 8, fields: ["id", "description", "scene", "duration", "camera", "associateAssetsIds"] })` 读取首批分镜行并识别**写入模式**（纯文本多参模式 / 分镜图辅助多参模式 / 首位帧模式）
...
3. 仅当当前 `storyboardTable` 窗口仍不足以写出 `videoDesc`、校对台词或判断连续性时，才按同批镜头最小补读 `script` 局部窗口；禁止在分镜表信息尚充足时先把整段剧本读入上下文
4. 切换到下一批 `storyboardTable` 行时，只补读新窗口引用的资产 ID；若必须补剧本，也只补同批镜头对应的局部窗口，禁止默认整表、整段剧本或整包 assets 一起带入

**跨批次连续性记录**：处理超过一个窗口（8行）的分镜表时，在切换到下一批分镜窗口前，必须记录上一批最后一条分镜的：
- 角色位置（画面左/中/右）
- 情绪状态
- 动作终态
并在下一批首条分镜的 `action` 字段中标注与上一批末镜的衔接关系（格式：`（承接上镜：{上一镜终态}）`），确保跨批次的视觉连续性。
5. **若为「分镜图辅助多参模式」或「首位帧模式」**：加载下方「分镜提示词 · 通用基础技法」与风格专属技法（激活 `director_storyboard`）作为提示词生成的全部参考依据，冲突时以风格专属技法为准；**若为「纯文本多参模式」**：跳过提示词相关技法加载
6. 确定分组（track）与时长规则：
...
7. **人物空间位置预分析**（纯文本多参模式跳过此步）：在已读取的分镜窗口内先建立人物位置基准；只有跨窗口连续性不足时才补读相邻窗口，而不是整表通读
8. **图像资产标注与正文绑定**（纯文本多参模式跳过此步）：为每条分镜的 prompt 生成图像资产标注前缀，按 `associateAssetsIds` 的引用顺序，依次标注 `@图N 为xx{类型}`；**提示词正文中所有涉及该角色/场景/道具的位置，必须使用对应的 `@图N` 替代其名称**，建立参考图与画面描述的直接绑定（详见下方「prompt 图像资产标注规则」）
9. **生成视频描述（videoDesc）**（所有模式均需）：根据 `storyboardTable` 当前行 + 该行精确读取到的关联资产信息（仅当前窗口引用的资产名称/类型/衍生状态），整合出结构化视频描述文本，填入 `videoDesc` 字段；信息足够时不得补读无关资产
10. 严格按 `storyboardTable` 的分镜数据行逐行写入分镜面板（排除表头与分隔行），根据模式差异化输出：
...
11. 写入完成后，仅返回一句确认：`已完成分镜面板写入（{当前模式名称}）`
...
- **videoDesc 必填（12维度）**（所有模式）：每条分镜的 `videoDesc` 必须包含以下 12 个维度，禁止使用「感到悲伤」等抽象情绪词或「柔光」等模糊光影描述：
  1. 画面描述（15-50字）
  2. 场景名称
  3. 关联资产名称
  4. 时长（秒）
  ...
  6. 运镜
  7. 角色动作（首帧预备状态）
  8. 情绪（具象可感描述，如"冷傲轻蔑""痛苦绝望"，禁止"开心""难过"等空泛词）
  9. 光影氛围（含光源方向+色调+明暗关系，禁止"柔光""暗调"等模糊描述）
  10. 台词（原文或「无台词」）
  11. 音效（或「无音效」）
  12. 关联资产ID
...
| 操作 | 调用 |
|------|------|
| 读取分镜面板 | `get_flowData({ key: "storyboard", fields: ["id", "index", "src", "state", "associateAssetsIds", "shouldGenerateImage"], limit: 12 })` |
| 读取关联分镜表 | `get_flowData({ key: "storyboardTable", ids: [分镜ID列表], fields: ["id", "description", "scene", "duration", "camera", "associateAssetsIds"] })` |
| 读取关联剧本窗口 | `get_flowData({ key: "script", lineStart, lineEnd, maxChars })` |
| 生成图片 | `generate_storyboard({ ids: [分镜ID列表] })` |

### 执行流程

1. 先最小读取分镜摘要：默认用 `get_flowData({ key: "storyboard", fields: ["id", "index", "src", "state", "associateAssetsIds", "shouldGenerateImage"], limit: 12 })`；若派发指令里已经给了明确镜头 ID，则改成 `get_flowData({ key: "storyboard", ids: [真实分镜ID列表], fields: ["id", "index", "src", "state", "associateAssetsIds", "shouldGenerateImage"] })`
...
4. 若需要在补图前复核镜头依据，优先只复读这批缺帧镜头的 `storyboardTable` 行：`get_flowData({ key: "storyboardTable", ids: [真实分镜ID列表], fields: ["id", "description", "scene", "duration", "camera", "associateAssetsIds"] })`；仅当镜头表仍不足以判断时，再按同批镜头对应的剧本窗口最小补读 `script`
5. 仅对这批缺帧镜头调用 `generate_storyboard({ ids: [真实分镜ID列表] })` 生成分镜图片（异步，发起即返回）
...
- 如需复核依据，必须优先沿用同一批真实分镜 ID 读取 `storyboardTable`；禁止先回到整张表或整段剧本
- 禁止重跑已有图片结果的镜头
...
含台词或高情绪镜头时，情绪必须写成可观察信号，如抿唇、眼眶发红、喉结滚动、手指收紧、呼吸发颤、声音压低。连续 3 条同强度情绪时，第 3 条必须递进或回落；高情绪镜头必须补 1 个峰值动作。
