...
class ProjectScriptsWorkbenchController {
  ProjectScriptsWorkbenchController({
    required this.ctx,
    required this.l10n,
    required this.token,
  ...
  Future<void> reloadScriptsAndStats(StateSetter setLocalState) async {
    ...
    setLocalState(() {
      session.infoLine = scriptList.isEmpty
          ? l10n.projectEditorScriptsWorkbenchReloadEmpty
          : l10n.projectEditorScriptsWorkbenchReloadCount(scriptList.length);
  ...
  Future<void> readContext(StateSetter setLocalState) async {
    ...
    setLocalState(() {
      ...
      session.infoLine = rows.isEmpty
          ? l10n.projectEditorScriptsWorkbenchReadContextEmpty
          : l10n.projectEditorScriptsWorkbenchReadContextCount(rows.length);
  ...
  Future<void> exportSelected(StateSetter setLocalState) async {
    ...
    if (selected.isEmpty) {
      throw StateError(l10n.projectEditorScriptsWorkbenchErrorNeedScriptIds);
    }
    final zip = await exportScriptsZip(token, selected);
    _setFollowUp(
      setLocalState: setLocalState,
      selectedIds: selected,
      actionSummary: l10n.projectEditorScriptsWorkbenchExportSelectedSummary(
        selected.length,
  ...
  Future<void> pollSelected(StateSetter setLocalState) async {
    ...
    _setFollowUp(
      setLocalState: setLocalState,
      selectedIds: selected,
      actionSummary: l10n.projectEditorScriptsWorkbenchPollSelectedSummary(
        selected.length,
        sample,
      ),
    );
  ...
  Future<void> extractSelected(StateSetter setLocalState) async {
    ...
    _setFollowUp(
      setLocalState: setLocalState,
      selectedIds: selected,
      actionSummary: l10n.projectEditorScriptsWorkbenchExtractSubmittedSelected(
        selected.length,
        accepted.status,
        accepted.message,
      ),
  ...
  Future<void> batchCreate(StateSetter setLocalState) async {
    ...
    if (count == null || count < 1 || count > 20) {
      throw StateError(l10n.projectEditorScriptsWorkbenchBatchCreateCountInvalid);
    }
    final prefix = session.addPrefixCtrl.text.trim().isEmpty
        ? l10n.projectEditorScriptsWorkbenchDefaultNewScriptName
    ...
    _setFollowUp(
      ...
      actionSummary: l10n.projectEditorScriptsWorkbenchBatchCreated(
        created.inserted,
      ),
    );
  ...
  String recommendedActionLabel() {
    return scriptBatchWorkbenchRecommendedActionLabel(
      l10n,
