...
List<String> summarizeScriptResultSnapshot(String? toolName, Object? result) {
  ...
  switch (normalizedTool) {
    ...
    case 'get_novel_events':
      final items = _extractResultItems(result);
      return items.isEmpty ? <String>['小说事件为空'] : <String>['小说事件 ${items.length} 条'];
    default:
...
List<ScriptWorkspaceArgumentSuggestion> buildScriptWorkspaceArgumentSuggestions({
  ...
  if (source == 'get_novel_text' && normalizedSelectedTool == 'get_novel_events') {
    suggestions.add(
      ScriptWorkspaceArgumentSuggestion(
        label: '沿用章节到事件',
        payload: <String, dynamic>{'novelId': ids.first},
