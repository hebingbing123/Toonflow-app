...
class _ScriptWorkbenchPanelState extends State<_ScriptWorkbenchPanel> {
  ...
  void initState() {
    super.initState();
    Future<void>.microtask(_refreshContext);
  }
  ...
  Future<void> _pollExtractState() async {
    ...
    setState(() {
      ...
      _extractStateLine = current == null
          ? '当前脚本提取状态为空：通常表示 idle 或已完成。'
          : '提取状态 ${current.extractState ?? 0}'
                '${(current.errorReason ?? '').trim().isEmpty ? '' : ' · ${current.errorReason}'}';
    });
  ...
  Future<void> _startExtractAssets() async {
    ...
    setState(
      () => _extractAssetsLine =
          '素材抽取已提交：${accepted.status} · ${accepted.message}',
    );
  }
            ...
            children: [
              ...
              TextButton(
                onPressed: _loadingContext || _runningAction
                    ? null
                    : _refreshContext,
                child: Text(_loadingContext ? '刷新中…' : '刷新上下文'),
              ),
            ],
          ),
          Text(
            _contextLine ?? '读取 get-script-api 上下文、导出 ZIP、轮询提取状态与发起素材抽取。',
