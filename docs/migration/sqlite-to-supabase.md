...
4. **归一化表与提升（本仓库已提供）**  
   ...
   INSERT INTO public.import_user_map (import_user_id, supabase_user_id)
   ...
   FROM public.promote_import_snapshots();
   ```
   未配置 `import_user_map` 时，项目仍会写入 **`owner_user_id` 为空**，客户端在 RLS 下**不可见**，直至补映射并再次执行 promote（`owner_user_id` 用 `COALESCE` 合并策略更新）。
