...
本文档描述 Toonflow 应用的监控和日志系统架构及使用方法。

## 概述

Toonflow 实现了全栈的可观测性解决方案，包括：
...
- `TOONFLOW_OTEL_EXPORT_ENABLED`: 启用 OTLP 导出（`1`/`true`/`yes`/`on`）
  ```bash
...
- `OTEL_SERVICE_NAME`: 服务名称（默认 `toonflow-server`）
  ```bash
  OTEL_SERVICE_NAME=toonflow-api
  ```
...
- `toonflow.metrics.api`: API 请求指标
- `toonflow.metrics.tts`: TTS 生成指标
- `toonflow.metrics.export`: 导出任务指标
- `toonflow.metrics.batch`: 批量操作指标
- `toonflow.metrics.db`: 数据库查询指标
- `toonflow.metrics.cache`: 缓存操作指标

可以通过 `RUST_LOG` 环境变量过滤特定指标：

```bash
# 只显示 API 指标
RUST_LOG=toonflow::metrics::api=info

# 显示所有指标
RUST_LOG=toonflow::metrics=info
```
...
export OTEL_SERVICE_NAME=toonflow-production
```
...
2. 检查环境变量：
   ```bash
   echo $TOONFLOW_OTEL_EXPORT_ENABLED
