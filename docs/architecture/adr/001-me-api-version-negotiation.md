...
2. **Accept Header Negotiation**: `Accept: application/vnd.openflow.me+json; version=2`

This ADR documents the chosen approach for Openflow's `/me` endpoint versioning.
...
```bash
# V1 (default)
curl -H "Authorization: Bearer $TOKEN" https://api.openflow.com/api/v1/me

# V2 (opt-in)
curl -H "Authorization: Bearer $TOKEN" https://api.openflow.com/api/v1/me?v=2
```
...
6. **Existing Pattern**: Openflow already uses query parameters for other optional behaviors
...
- Inconsistent with Openflow's existing API patterns
