...
**阅读提醒**：本文兼具 **目标架构说明** 与 **长周期进度串** 两种角色；其中部分段落会保留历史兼容接口、numeric id、probe/workbench 演进记录。引用 project/script/jobs scope 现状时，应优先对照 [`product-deep-links.md`](../product-deep-links.md)、[`harness-ws-context-matrix.md`](./harness-ws-context-matrix.md)、[`tasks-http-api-cleanup.md`](./tasks-http-api-cleanup.md) 与 [`workspace-team-full-plan.md`](./workspace-team-full-plan.md) 的较新 UUID-first 结论。
