...
todos:
  ...
  - id: harness-rust-core
    content: 在 Rust 中落地 Harness 分层（工具/权限/观测/Agent 循环），替换 vm2 类沙箱为进程或 WASM 等硬隔离方案；**进度**：`ToolRegistry` 静态目录；**HTTP** **`GET /api/v1/harness/tools`** 在 **`backend/src/harness/http.rs`**（与 **`GET/PUT/POST/DELETE /api/v1/skills*`**（读、覆盖已存在文件、新建文件、删单文件）解耦）；**WS** 载荷类型在 **`harness/wire.rs`**；**WS** 实现集中在 **`backend/src/harness/ws/`**（**`upgrade`**, **`connection`**, **`dispatch`**, **`auth`**, **`outbound`**, **`tool`**, **`agent`**（`HarnessAgentWsParams`）, **`chat`**, **`channel`**, **`session`**）；**observe** 挂钩 **memory REST**、**job worker**（**claimed** / **succeeded** / **failed** / **cancelled**）；**`permissions`**：**`HARNESS_WS_CHANNELS`** 限制 **`script`** / **`production`** attach；**WS** `harness.tool.invoke` / `harness.tool.result`（**`echo`**、**`isolated.echo`**（子进程 + **`HARNESS_ISOLATE_MAX_CONCURRENT`** 并发槽）、**`skills.read`**、**`wasm.probe`**（**wasmi** + `build.rs` 嵌入 WASM））、**WS** **`harness.agent.run`** / **`agent.chat.send`**；文档 **`docs/websocket-events.md`** / OpenAPI 摘要已对齐。**仍缺**（产品化加深）：通用「用户上传 WASM」沙箱与策略、进程池预热/回收、与 LLM 流式工具调用融合、完整观测/trace 管线；**域功能**继续按竖切迁入 Rust 并挂 Harness 策略/观测
    status: completed
  - id: rust-backend-mvp
    content: Rust 后端 MVP：**主库仅为 Supabase Postgres**（§4.1；SQLx 直连）；旧 SQLite 仅迁移源；AI Provider 流式；**竖切**：Flutter 已接项目/剧本/分镜 REST（**`POST` 新建项目**、**`POST` 项目下新建剧本**、**`POST` 剧本下新建分镜**、列表 + 按 numeric id `GET/PATCH`、**`DELETE` 项目/剧本/分镜**）；**项目统计**：**`GET /api/v1/projects/summary`**（用户维度项目/剧本/分镜/**小说**/**角色（role 资产）**/**画风**/素材总数，**`video_count`** 占位 **0**）、**`GET /api/v1/projects/{project_id}/stats`**（**`project_id`** UUID）（单项目；**`role_count`**/**`novel_count`** 来自 PG，**`video_count`** 仍为 0，对齐旧 **`generalStatistics`** 命名）；**静态模型目录**：**`GET /api/v1/models`** / **`GET /api/v1/models/detail`**（`data/models_catalog.json` 编译嵌入；旧 **`modelSelect`** 过滤语义）；**任务**：`app_generation_job` + REST（**`GET /api/v1/jobs`** 可选 **`kind`**/**`status`** query）+ **`jobs::worker`**（**PG + SKIP LOCKED** 已可多实例）、取消/重试/幂等、**WS** `generation.job.updated`；**GET /api/v1/me** 含 **`plan_tier`**；**用量**：`app_usage_event` + **`GET /api/v1/usage/summary`**（总量 + **`event_counts_last_7d`**）；**Agent 记忆**：`app_agent_memory` + **`POST /api/v1/agents/memory/query|clear|append`**（旧 getMemory/clearMemory 语义；append 无自动摘要）；**可观测**：`X-Request-Id` + 错误 JSON `request_id`；**`GET /api/v1/version`** 可选编译期 **`git_sha`**（**`OPENFLOW_GIT_SHA`**）；**契约**：OpenAPI/WS 与集成测试（isolate）；**进度**：**`backend/src/app/mod.rs`** 内 **`contract_smoke_tests`**（**无 DB**）：**`/health`**、**`/api/v1/health`**、**`/api/v1/ping`**、**`/api/v1/version`**、**`/api/v1/ready`**；**Supabase 形 HS256 JWT**：**`/api/v1/models`**、**`/api/v1/models/detail`**、**`/api/v1/harness/tools`**、**`/api/v1/skills/summary`**、**`/api/v1/skills`**、**`/api/v1/skills/content`**（**GET**）、**`PUT /api/v1/skills/content`**（**401**/**400**）、**`POST /api/v1/skills/content`**（**401**/**400**/**409**）、**`DELETE /api/v1/skills/content`**（**401**/**400**/**404**/**204**）、**`GET …/projects/summary`**（无 Bearer → **401**）、**`GET`/`POST …/projects/{project_id}/assets`**、**`POST …/assets/corner-scape`**（**`page`/`limit`** + 组合 **`script_legacy_id`/`asset_type`/`name`** 烟雾）、**`GET`/`PATCH`/`DELETE …/projects/{project_id}/assets/{aid}`**、**`PUT`/`DELETE …/projects/{project_id}/scripts/{s}/assets/{a}`**、**`GET`/`POST`/`PATCH`/`DELETE …/projects/{project_id}/novels`**、**`GET`/`POST …/art-styles`**、**`POST …/art-styles/extract-prompt`**（无 Bearer → **401**；空 **`images`** → **400**；合法体、无 **LLM** → **503** `llm_not_configured`）与 **`GET`/`PATCH`/`DELETE …/art-styles/历史/{id}`**（无 DB → **503** `database_error`）；**`app::pg_contract_tests`**（**`#[ignore]`**，需 **`DATABASE_URL`** + **`SUPABASE_JWT_SECRET`** + 已迁移库：**`POST /api/v1/projects`** → **`GET …/stats`** → **`GET …/assets`** → 剧本、**`GET …/assets/{aid}`**、**`asset_type`/`name`** 筛选/关联/分页 → **`DELETE` 取消关联** → 筛选为空 → **`/novels`** CRUD + **`search`** 列表 → **`DELETE` 项目** → 404；**`promote_staging_populates_assets_and_links`**；**`art_styles_crud_roundtrip`**）；**不依赖 Redis 才能完成重构**；**重构分支**：`harness::ws`、`app/{handlers,router}`、`jobs/worker`、`billing/*`、`state/*`、`auth/*`、`error/*`、`llm/envelope` 等已模块化；**`POST /api/v1/webhooks/billing`** 无 **DB** 烟雾（无效 HMAC → **401**；**`BILLING_WEBHOOK_SECRET` + 正确 `x-openflow-signature`**、无 **pool** → **503** **`database_error`**；并行测试用 **`tokio::sync::Mutex`** 串行）；**仍缺**更广的端到端契约回归矩阵与业务域全覆盖
    status: completed
  - id: product-shipping-bar
    content: **完整产品切换**总验收（**不等于**仅 `rust-backend-mvp`）：**功能 parity 主表**见 **[`electron-node-parity.md`](./parity-audit.md)**（随 `src/router.ts` 变更维护）；扩充 **端到端契约 + 集成回归**（无 DB 烟雾、需 PG 的测试、关键用户路径）；将 **`jobs-and-webhook-hardening`**、**`saas-product-spec`**、**`quality-bar`** 中 **blocking 上线** 的子项纳入同一门禁；**灰度与回滚**策略可执行；**完成态** = **`decommission-electron`** 具备条件（旧栈可下线）。**进度**：以 `rust-backend-mvp` + `flutter-shell` 为底座，按 parity 文档 **分波（B–H）** 竖切补齐。已完成：PG contract tests for billing webhook events audit, jobs REST, prompts, skills, vendor credential, asset image file, production workbench, novel events, quality reviews, tasks workbench, project CRUD, general project update, scripts, storyboards, art-styles, settings danger ops CLI。
    status: completed
  - id: postgres-ops
  ...
  - id: supabase-auth
    ...
    status: completed
  - id: flutter-shell
    content: Flutter 桌面+Web 均以可配置 baseUrl 连接 Rust；默认端口 8666；dev 例 http://127.0.0.1:8666，prod 为部署 URL；CORS/WebSocket/鉴权与后端一致；**竖切**：首页调试区已接项目/剧本/分镜 REST、**jobs** 列表/**`GET /api/v1/jobs/{id}`**/**`GET jobs?kind=flutter.probe`**/**`GET jobs?status=failed`**/**`GET jobs?kind=flutter.probe&status=queued`**/**`GET /api/v1/jobs/kinds`**/**`kinds/summary`**/**`status/summary`**/探针创建，与 **`GET /api/v1/skills/summary`** / **skills** / **harness/tools** 只读探针、**WS** `harness.tool.invoke` / **`harness.agent.run`**（Flutter 探针：attach + 多轮工具；**echo**、**isolated.echo**、**skills.read**、**wasm.probe**，与 Skill path 联动）；新增 **`agent_workspaces_section`** 作为 script/production 工作台入口（attach + `harness.agent.run` + `get_flowData` probe 收口），其中 production 工作区已把最新工具结果继续收口为 flow 摘要与“下一步建议”卡片，可按 `assets` / `scriptPlan` / `storyboardTable` / `storyboard` 状态自动推荐读取 flow、切换子代理与填充提示词，并支持在建议卡上一键读取 flow 或运行子代理；本轮再新增固定的 `scriptPlan → assets → storyboardTable → storyboard` 阶段看板，按空白/待补图/建议刷新/已完成状态显示当前制作环节，并可直接推进对应阶段动作；script 工作区也同步新增固定的 `storySkeleton → adaptationStrategy → novel context → script content` 阶段看板，按待生成/待读取/已就绪/已完成状态显示当前改编环节，并可直接读取上下文或推进对应子代理；调试区含 **`/health`**、**`/api/v1/health`**、**`/api/v1/ping`**、**`/api/v1/version`**、**`/api/v1/ready`**、**`/api/v1/usage/summary`**、**`GET /api/v1/models`** / **`models/detail`**（静态目录）、**`agents/memory/query`**（`projectUuid` 优先，legacy `projectId` 回退）；**`POST /api/v1/projects`** 空项目探针；Projects 页除 **`GET …/projects/summary`** 汇总与 **`GET …/art-styles`** 列表外，现已新增“画风工作台”，可统一执行 **`GET/POST/PATCH/DELETE …/art-styles*`**、读取 **`GET …/art-styles/历史/{id}/cover`** 本地封面并调用 **`POST …/art-styles/extract-prompt`** 生成 prompt；同时也已新增“创作手册工作台”，统一承载导演手册与视觉手册的刷新、查看、创建、更新、删除，并用 `GET/POST /api/v1/visual-manual` 与 历史 project 手册写接口形成首页产品入口；Projects 主区还新增了“记忆工作台”，可按项目 **`projectUuid`**（优先）或 legacy numeric **`projectId`**、agent type 与可选 episodes id 查询、追加、清理 Agent 记忆，旧首页首项目 memory probe 下沉到兼容性折叠区；“质量评审”主区也已升级为正式工作台入口，可在一个对话框中完成评审筛选、坏例查看、统计/阶段通过率读取、详情查询与手动创建，旧 probe 创建入口下沉到兼容性折叠区；“任务中心”主区现也已升级为正式工作台入口，可在一个对话框中统一完成任务项目/分类读取、按项目/分类/状态筛选列表，以及按 UUID 或 legacy numeric task id 打开详情，旧加载与详情 probe 下沉到兼容性折叠区；项目详情对话框展示 **`GET …/projects/{project_id}/stats`**、**`GET …/novels`**（**`fetchProjectNovelsByProjectId`** 与 **POST/GET/PATCH/DELETE** 探针），其中“小说与事件”主视图已进一步精简为章节/事件正式工作台摘要卡，旧 REST 首条/末条探针退出主链路，仅在兼容性折叠区保留回归入口；章节工作台已把 `get-novel-data` / `get-novel-index` / `get-novel-event-state` / `batch-delete` 等 历史 包装读写收口为正式批量动作；**`GET …/assets`**（**`fetchProjectAssetsByProjectId`** / **`ListAssetsResponse`**；**`page`/`limit`** 分页探针；**`asset_type`/`name`** 筛选探针；**`script_legacy_id`+`page`/`limit`** 与下拉联动探针）、**`GET …/assets/{aid}`**（**`fetchProjectAssetByProjectIds`**）与刷新按钮，及资产正式工作流表单（显式目标选择的 **POST/PATCH/DELETE**、**PUT/DELETE** 剧本–资产关联与高级筛选），并新增“资产历史图工作台”用于 `corner-scape` 资产查询与历史图预览、“上传 Clip 资产”正式表单用于 `upload-clip` 入库、“资产图片工作台”用于 `GET/POST/PATCH/DELETE …/assets/{aid}/images*` 与预览，以及“资产出图工作台”用于 `production/assets/get-assets-data` 摘要、历史 `get-material-data` / `batch-generation-data` / `polling-prompt-assets`、`batch-generate-assets-image`、`polling-image`、`delete-assets-derivative` 与单资产 URL 更新；原首条/末条资产动作保留在兼容性折叠区；项目剧本区已新增“剧本批量工作台”，可按名称读取 `get-script-api` 上下文、显式选择目标剧本 id，并在同一入口内完成批量导出、提取状态轮询、素材抽取与批量创建；剧本编辑对话框除 **`DELETE`** 外，已升级为 script/workbench 正式入口，可直接读取当前脚本上下文、查看关联素材摘要、导出 ZIP、轮询提取状态并发起素材抽取，并新增“编辑图片工作台”用于加载 edit-image flow 模板与默认模型、上传源图、保存/更新 flow 步骤状态以及发起 `generate-flow-image`；分镜列表对话框已新增“分镜出图工作台”，可在剧本上下文内批量勾选分镜、沿用现有提示词发起 `batch-generate-image`、读取当前预览图与下载链接并导出所选分镜 ZIP；并在增删剧本后尝试刷新 stats；可 **`POST 空剧本`**、**`DELETE`** 项目；分镜列表对话框可 **`POST 空分镜`**；分镜编辑对话框除 **`DELETE`** 外，已升级为 storyboard/workbench 正式入口，可直接读取/保存预览图、管理视频轨道、生成默认视频提示词、提交视频任务并切换当前候选视频
    status: completed
  - id: master-detailed-parity-audit
    content: 基于 **`master`** 做逐项详细对比（旧 **`src/routes/**`**、**`src/socket/**`**、**`src/agents/**`**、前端调用面与当前 **`backend/`**、**`frontend/`**、OpenAPI、WS 文档逐一核对），把“接口已迁移”“设计性替代”“真实遗漏”“前端仅 probe 未完成产品 UX”的边界重新厘清；审计结论见 **[`master-detailed-parity-audit.md`](./detailed-parity-audit.md)**。已收口明确 REST 漏项（`production/edit-image/upload-image`、`scripts/batch-add`）、script 侧核心只读域工具（`get_planData`、`get_script_content`、`get_novel_text`、`get_novel_events`）、production 侧核心域工具（`get_flowData`、`add_deriveAsset`、`del_deriveAsset`、`generate_deriveAsset`、`generate_storyboard`）以及 script/production 子 Agent 编排工具（`run_sub_agent_*` / `run_supervision_agent`）；Flutter `agent_workspaces_section` 已升级为 script/production 双工作区任务化入口（模板提示词、子 Agent 快捷选择、script/prod 域工具均支持 JSON 参数探测、WS 最近事件摘要），并补齐最小结果回写闭环（script 侧优先回写 `get_script_content.content`，并支持 `run_sub_agent_script` 结果直写候选，且支持基于 `get_planData` 一键回写 `set-plan-data`；同时 script 工作区现已根据 `get_planData` / `get_novel_text` / `get_novel_events` / `get_script_content` 自动生成下一步建议卡，直接读取上下文或运行对应子代理，并把 `get_novel_text` / `get_novel_events` 的原始 `items` 数据直接渲染为章节/事件快照，附带 `novelId` 一键填参；production 侧支持通用工具结果写回扩展 key，核心 key 在 `get_flowData` 直写外，还支持资产/分镜变更工具与 production 子 Agent 工具先刷新对应 flow key 再安全回写，并提供建议 key 快捷应用；读取 `assets` / `storyboard` flow 后还能直接从当前快照生成 `generate_deriveAsset` / `generate_storyboard` 的 `ids` 参数，以及 `add_deriveAsset` / `del_deriveAsset` 的资产树模板参数）；剧本编辑器已进一步收口为 script workbench 入口，可直接处理脚本上下文、导出、提取状态与素材抽取；剧本分镜编辑器已进一步收口为 production storyboard/video 工作台入口，可直接处理预览图、轨道、默认视频提示词、视频生成提交以及候选视频选择/删除，并把 workbench 逻辑拆为独立职责文件以控制单文件复杂度；并在 UI 形态上新增 Agent Workspace 二级导航（Script / Production / Activity）与执行活动面板，将双工作区卡片拆分为独立职责组件文件，减少单文件复杂度并提升可维护性。
    status: completed
  - id: decommission-electron
    content: 功能 parity 与灰度后 **下线** Electron + Node 服务端路径；**并清理仓库内旧实现**（见 **§11.1.1**、**[`electron-node-parity.md`](./parity-audit.md)** §7）：删或归档 **`src/`** 下 Express/Socket.IO 服务端、根 **`package.json`** 中仅旧栈的依赖与 scripts、仅旧栈的 CI job；**保留**仍需要的 **`openflow-sqlite-import`** 等迁移工具至明确路径；README/发布以 **`backend/` + `frontend/`** 为准。已完成：删除 src/ 目录（178 个 .ts 文件/14.4k+ 行）、scripts/main.ts & build.ts、electron-builder.yml、Dockerfile、tsconfig.json、yarn.lock、data/web/ & data/serve/；简化 package.json 至仅 lint/license/refactor:check；更新 README 反映新架构
    status: completed
  - id: quality-bar
  ...
  - id: saas-product-spec
    content: SaaS 规格（§12）：首期 CNY 收银与 plan_tier；后期 USD（Stripe/Paddle 等）；billing_currency/provider 预留；积分与 webhook；用量/审计 Schema（§12.3）；org/合规按阶段；**进度**：`app_user_profile`；计费 webhook upsert；**`app_usage_event`** 追加 + **`GET /api/v1/usage/summary`**（含近 7 天 **`event_type`** 分组 **`event_counts_last_7d`**）；**配额硬执行（plan_tier 联动）已落地**；**订阅状态机已补字段与顺序保护**（`subscription_status` / `subscription_current_period_end_at` / `subscription_status_updated_at` + webhook 解析/写入 + 乱序事件时间戳保护）；**provider 解析入口已抽象并表驱动化**（已含 Stripe + Alipay + Paddle，且补充 Stripe/Paddle 高频事件映射，含 Stripe `invoice.paid -> active`、`invoice.overdue -> past_due`、`invoice.payment_reversal -> past_due` 与 `customer.subscription.unpaused -> active` fallback、Paddle `subscription.expired -> canceled` fallback）；**Stripe/Alipay/Paddle 长尾信息事件白名单继续扩展**（Stripe 追加 `pending_update_created`、`payment_intent.canceled|processing|requires_action`、`charge.captured|refunded|refund.updated|dispute.*`；Paddle 追加 `transaction.paid|ready`，均不触发状态迁移）；**事件字段兼容已增强**：状态推导与审计元数据提取支持 `event_type` 回退键，审计时间支持 `event_created_at` 回退键；**可观测性已补**：`app_billing_webhook_event` 入库 `is_informational_event`，并在 webhook 响应回传 `informational_event`，同时新增 **`GET /api/v1/webhooks/billing/events`** 支持按 `informational_event`/`provider`/`raw_event_id`/`raw_event_id_prefix`/`event_type`/`provider_event_id`/`provider_event_id_prefix`/`event_created_from`/`event_created_to`/`id_min`/`id_max`/时间窗口过滤审计查询，并支持 `sort=id_desc|id_asc`，响应返回 `total` + `has_more` + `next_offset` 便于分页；**CNY/USD 适配层已落首版并支持配置**（缺省 `billing_provider` 时按 `billing_currency` 路由；默认 `CNY→alipay`、`USD→stripe`，可由 `BILLING_CURRENCY_PROVIDER_MAP` 覆盖；仅用于状态解析，不改变去重命名空间）；**OpenAPI 已补 webhook 事件族覆盖说明**（Stripe/Paddle 常见 subscription+invoice/transaction fallback）；**写入前状态合并守卫已落地**（阻止 provider 派生事件把终态误回滚，显式状态更新仍可覆盖）；**事件级置信度守卫已补**（provider 字段直出优先于 event fallback，等时间戳下防止低置信度状态抖动）；**webhook 去重键已升级为 `<provider>:<id>`**，并记录 `provider/raw_event_id/event_type/event_created_at` metadata；响应返回 `provider_event_id`；**`GET /api/v1/me` 已暴露 subscription 字段**（便于前端与运营读当前状态）；**仍缺**生产级收单适配细节
    status: completed
  - id: jobs-and-webhook-hardening
    content: 长时生成：**已完成**：`app_generation_job` + REST + worker（**`SKIP LOCKED`** + **`claimed_by`**）+ **WS** `generation.job.updated`；**`GET /api/v1/jobs`**（可选 **`kind`**/**`status`** 精确过滤；**`limit`**/**`offset`** 分页）、**`GET /api/v1/jobs/kinds`** / **`kinds/summary`** / **`status/summary`**；**queued/running 取消**、`failed` **重试**、`Idempotency-Key`；计费 webhook HMAC/去重/验签；审计 **`GET …/webhooks/billing/events`**。**刻意后置**：Redis/云队列（非必需）、限流极致化
    status: completed
isProject: false
---
...
**建议阅读顺序**：**§0–§6**（目标与取舍）→ **§11**（单仓与 API/WS/CI 默认）→ **§7–§9**（已确认前提、仍待拍板、速查索引）→ **§8**（分支）→ **§12–§13**（SaaS/计费与工程加深项）。**§4.x** 在涉及库与 Auth 时细读。文首 YAML 的 **`product-shipping-bar`** 汇总「完整产品切换」门禁（parity、回归、blocking 子项、灰度），**完成态与 `decommission-electron` 对齐**，与 **`rust-backend-mvp`**（后端首条验收）区分。**Parity 主表**：[**`electron-node-parity.md`**](./parity-audit.md)。**按方向拆分的进度索引**（仓库、契约、后端、Harness、任务与计费、质量、Flutter、发版 parity）：[**`roadmap-index.md`**](./index.md)。任务队列：**默认仍以 Postgres 为真源**（§7.1、YAML `jobs-and-webhook-hardening`）；**Redis / 云托管旁路队列仅在量化瓶颈 Gate 通过后立项**，执行拆解见 [**`roadmap-jobs-saas.md`**](./jobs-saas.md)，避免将「拆分路线图里的必做」误解成「默认必须上 Redis」。

**阅读提醒**：本文兼具 **目标架构说明** 与 **长周期进度串** 两种角色；其中部分段落会保留历史兼容接口、numeric id、probe/workbench 演进记录。引用 project/script/jobs scope 现状时，应优先对照 [`product-deep-links.md`](../product/deep-links.md)、[`harness-ws-context-matrix.md`](../features/harness/ws-context-matrix.md)、[`tasks-http-api-cleanup.md`](./tasks-http-api-cleanup.md) 与 [`workspace-team-full-plan.md`](../features/workspace/team-collaboration.md) 的较新 UUID-first 结论。
...
**命名与迁移历史约定**：新文档与新实现优先使用 **historical/import-era** 术语；`supabase/migrations` 中历史 `legacy_*` 名称不做回写改历史，统一通过前向迁移收敛当前 schema。见 **[`database-migration-history-policy.md`](../migration/database-migration-history-policy.md)**。
...
| 主题 | 已定锚点 | 仍待产品/运维拍板 |
| ---- | -------- | ---------------- |
| 目录与 Monorepo | §11.1、§11.8 | 实验路径（如 `openflow-server/`）在重构分支 **迁入 `backend/` 或删除** |
...
5. **刻意保留（直至迁移窗口结束）**：**`openflow-sqlite-import`**（`backend/src/bin/sqlite_import.rs`）等 **SQLite → Postgres** 工具可暂留；若不再执行迁移，再移至 **`scripts/`** 或单独 repo 并更新文档。
...
- **契约**：**OpenAPI 3.x** 为单一事实来源（Rust/utoipa 合并 `shell.rs`、路径索引与 handler 注解；`GET /api/v1/openapi.yaml` / `export-openapi`）；错误体 **稳定结构**（如 `code` + `message` + `request_id`）。
