...
| **迁移工具 `openflow-sqlite-import`** | `backend/src/bin/sqlite_import.rs` | 通常 **保留**（离线 SQLite → staging，非在线 HTTP） |
...
| 文件 | 说明 |
|------|------|
| `backend/src/bin/sqlite_import.rs` | SQLite → PG staging 导入 CLI（**`cargo run --bin openflow-sqlite-import`**），**不等于**在线 HTTP |
...
| **多语言 README** | 文档仅 **简体中文**（根 `README.md`）与 **English**（`docs/README.en.md`）；统一指向 **`LICENSE`** + `backend`/`frontend`/路线图链接。 |
