...
| 内容 | 状态 | 备注 |
|------|------|------|
| 用户上传 WASM 策略（配额、签名、审计） | `shipped`（核心） | **已交付**：`validate_user_wasm_upload` + **`HARNESS_USER_WASM_MAX_BYTES`** + **`POST /api/v1/harness/user-wasm/validate`**（JWT）+ **`POST/GET/DELETE /api/v1/harness/user-wasm`**（`revoked_at` 软删除；列表/配额仅 active）+ WS **`wasm.user.probe`** + fuel **`HARNESS_USER_WASM_FUEL_LIMIT`**（单测 `user_probe_fuel_exhaustion_returns_err`）+ 超时 **`HARNESS_USER_WASM_INVOKE_TIMEOUT_MS`**（`wasm_timeout`）+ **`HARNESS_USER_WASM_DISABLED`** + 审计表 `app_harness_user_wasm_audit` + **`harness_user_wasm_signal`** + object_store 半缺失保护与错误码收敛。威胁模型与威胁→验证映射见 [`harness-user-wasm-threat-model.md`](./harness-user-wasm-threat-model.md)。**仍为 next**：跨环境告警编排与自动化（阈值、通知、运行手册）。 |
...
| Trace / 结构化观测 + OTel 导出 | `shipped`（gRPC OTLP traces） | **必做**；与 `quality-bar`、运维 KPI 联动；**已实现**：`OPENFLOW_OTEL_EXPORT_ENABLED` + **`OTEL_EXPORTER_OTLP_*`** gRPC 导出 + `telemetry` 模块 + README + 单测 / ignored collector 烟测；采样率 / PII 白名单等见 WP-F PR 切片仍 **`next`** |
...
| **PR 切片** | （0）投递前 **`validate_user_wasm_upload`** + **`POST /api/v1/harness/user-wasm/validate`**（已完成）；（1）设计与威胁模型（[`harness-user-wasm-threat-model.md`](./harness-user-wasm-threat-model.md)）；（2）迁移 **`app_harness_user_wasm`** + **`POST/GET/DELETE /api/v1/harness/user-wasm`**（已完成：可调 **`HARNESS_USER_WASM_MAX_STORED_PER_USER`**、**`HARNESS_USER_WASM_LIST_CAP`**）；（3）WS **`wasm.user.probe`**（已完成：active-only + revoked 不可执行）；（4）资源保护（已完成：fuel 限额 **`HARNESS_USER_WASM_FUEL_LIMIT`** + invoke 超时 **`HARNESS_USER_WASM_INVOKE_TIMEOUT_MS`**）；（5）运维开关（已完成：**`HARNESS_USER_WASM_DISABLED`**，内建 **`wasm.probe`** 继续使用 **`HARNESS_WASM_PROBE_DISABLED`**）；（6）结构化审计日志（已完成：validate/persist/revoke/invoke success/fail）；（7）审计持久化薄切片（已完成：`app_harness_user_wasm_audit` + Rust 读写 helper + 定向测试）；（8）对象存储 offload（进行中：已完成最小可用接线、fallback 与定向后端验证；新增半缺失配置保护，避免语义漂移）；（9）最小可执行告警信号（已完成：统一 `harness_user_wasm_signal` 字段键 + 关键失败路径接线 + 定向断言）。跨环境告警编排与自动化通知仍为 **`next`**。 |
...
| **PR 切片** | （0）**stub**：~~`OPENFLOW_OTEL_EXPORT_ENABLED` 时仅 warn~~ **已替换**；（**OTLP gRPC traces**）`OPENFLOW_OTEL_EXPORT_ENABLED` + `OTEL_EXPORTER_OTLP_ENDPOINT` / `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` + `telemetry::init_tracing_subscriber` + README + 测试；（1）统一 `tracing` span 与 baggage 约定；（2）**必做**：可配置导出（环境区分 staging/prod，采样率可调但 prod 非零）；（3）**必做**：PII 脱敏与字段白名单评审。 |
