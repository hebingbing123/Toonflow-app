...
- [x] **方案 B**（可选）：**Backend** `GET /api/v1/jobs/queue/stats` + **`OPENFLOW_INTERNAL_OPS_TOKEN` / `X-Openflow-Internal-Token`** + OpenAPI + Flutter 只读卡片（`INTERNAL_OPS_TOKEN` dart-define）— 见 `backend/README.md`。
