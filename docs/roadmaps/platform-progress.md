...
**阅读提醒**：本文是**执行进度串**，会同时出现当前产品语义、兼容层收口记录和未来轨道说明。凡涉及 project/script/jobs scope，优先按较新的 **UUID-first** 真源理解，再把这里当作“什么时候落到哪一步”的时间线。
...
- **W4.5 已起步**：`app_generation_job` 先按规范化 **`project_uuid`**（首选）/ **`project_numeric_id`**（legacy fallback）派生 workspace 可见范围，`GET /api/v1/jobs/page`、`GET /api/v1/jobs`、jobs 三个 summary 端点，以及 jobs 详情/取消/重试链路已统一到 owner 或同 workspace 项目成员可见（未传 project scope 的列表/汇总仍保留 owner 个人视图）；`POST /api/v1/jobs` 在 payload 带 project 字段时会做成员校验并规范化 `project_uuid`/`project_numeric_id`；本地 artifact 下载目录与更新通知已改按 `job.owner_user_id` 对齐；worker 侧 `video/voiceover/asset-image` 的项目写回查询已切到 workspace 成员校验；并完成书面定稿：`app_generation_job` 暂不新增 `workspace_id`，由规范化 `project_*` 派生 workspace 可见性
...
- **最新补齐**：平台通知中心 **P-A1** 已落地 — 新增 `app_notification` 与 settings inbox API（`GET /api/v1/settings/notifications`、`POST /mark-read`、`POST /mark-all-read`），并通过 WS `settings.notification.created` / `settings.notification.updated` 把 skills 变更、jobs 终态、workspace invite 生命周期推到 Flutter 主导航「通知中心」pane；支持未读计数、列表筛选、已读回写与深链打开 jobs / team workspaces / projects，其中 jobs / projects 深链已按产品壳 UUID-first scope 恢复链路对齐
- **最新补齐（F1.4 / 契约文档同步）**：`docs/websocket-events.md` 与 `backend/src/openapi_spec/ws_protocol_description.md` 已补齐通知中心实时 producer 清单，明确 `settings.notification.created` / `updated` 现在不仅覆盖 skills / jobs / workspace invites，也覆盖 **`content_compliance_alert`** 与 **`content_compliance_alert_cleared`** 两类合规通知，避免前端或运维按旧清单误判 WS 推送来源
...
- **HTTP 收敛 H0**：[**`http-api-cleanup-h0-inventory.md`**](./http-api-cleanup-h0-inventory.md) 已落盘（`rg` 基线 + parity 读法）。**H1**（`agent_memory` / `clear-agent-memories`：**`projectUuid`** 优先 + legacy **`projectId`**）已合。**H2**（`asset_extract`）已合。**H3**（**`assets-generate`** 入队 payload **v2**）已合 — [**`assets-generate-job-payload-v2.md`**](./assets-generate-job-payload-v2.md)。**H4**（Harness WS **`projectUuid`** / **`scriptUuid`** + legacy **`project_id`** / **`script_id`**；矩阵 [**`harness-ws-context-matrix.md`**](./harness-ws-context-matrix.md)；Flutter `agent_workspaces` 双写）已合。**H5·C0–C4**：C0–C3 见 **`tasks-http-api-cleanup.md`** H5；**C3**：`cargo clippy --all-targets --all-features -D warnings` 清洁批次（**`b306fe98`**）；**C4**：删除未注册孤儿模块 `video_encoder.rs` 已完成。**H5·D kickoff** 已完成首轮盘点：`production`/`task_center`/`agent_workspaces` 与 jobs payload、`import_staging` / `promote_import_snapshots()`、`pg_contract_tests` 仍大量依赖整型兼容语义，因此继续保留为 **独立 schema 窗口**，暂不与普通功能 PR 混做。**H3.5**（D-batch 数据迁移评估）已完成：创建 **job payload `project_uuid` backfill** 迁移（`20260527120000_backfill_job_payload_project_uuid.sql`），为后续 D-batch 列移除做准备；评估文档见 [**`.tmp/H3.5_migration_evaluation.md`**](../../.tmp/H3.5_migration_evaluation.md)；**关键发现**：`promote_import_snapshots()` 函数与 contract tests 仍依赖 `numeric_id` 冲突解析，列移除需独立 schema 窗口 + DBA 批准（预计 6-9 个月）。其后续前端首刀已落在 **task center**：失败任务 deep-link 与产品壳跳转现已透传 `project_uuid` / `workspace_id`，为后续移除“仅靠 numeric 项目号恢复上下文”的链路先清一段路；随后 **agent workspaces** 也开始在项目切换时主动清空未显式提供的旧 `scriptId/scriptUuid`，避免旧 numeric/script scope 残留到新的 Harness attach；`short video space` 选项目时同步到产品壳的也不再只是 `projectNumericId`，而是 `ShortVideoProjectScope(projectNumericId, projectUuid, workspaceId)`；此前 `WorkspaceWritebackController.writeBackScriptWorkspaceResult()` 已改为优先使用 `projectUuidController`，`writeBackScriptPlanWorkspaceResult()` / `writeBackProductionFlowResult()` 也能在缺少 numeric `projectId` 时先从 `projectUuidController` 解析 `project.numericId`；最近又把 `system_probes/models_catalog` 的 production probe、script-agent probe，以及 `assets-generate` 的 generate/polish/batch probe 全部接上当前 project scope，不再默认写死 `projectId: 1`；这轮继续把 probe 的假资源号往后推了一层，新增 `projectUuid + scriptId -> asset/storyboard numeric id` 解析 helper，让 production probe、storyboard polling/export、typed production probe、`assets-generate` probe 与 `script-agent/update-data` 优先复用当前 workspace/project/script 上下文，而不是继续固定 `1`；顺手把 `account` probe 里的 `clear-agent-memories` 也收口到当前 workspace 输入，避免它再默认抓“第一个项目”或裸回退 `1`；最新补刀则把 `account` probe 的 clear scope 也收成 UUID-first，显式给出 `projectUuid` 时前端不再再挂一个 fake numeric `projectId = 1`，probe note 也会按 UUID / numeric 实际来源回显；`projects` 面板里的 agent-memory compat probe 现在也优先从 `ProjectRow.id` 构造 UUID-only project ref，只有项目 UUID 真缺失时才回退 numeric `projectId`；小说托管抓取的 `crawl-schedules` compat caller 现在也直接复用项目 UUID path，不再为已有 `project.id` 的 schedule create 额外双传 `project_numeric_id`；再往前补一格后，`settings` probe 里的 `agent-deploy/deploy-model` 也改为先读 `agent-deploy/list` 解析真实 deploy id，不再固定 `id: 1`；最新六刀则把小说 workbench / events compat wrapper 改成 `projectUuid` 优先，项目编辑器已有 UUID 的调用面不再多绕一次 `projectIdForNumericId()`，并把 `delete-novel` / `update-novel` compat 调用也收口到可直传 `projectUuid`，避免继续扫描全项目列表；`project/compat.dart` 里的 `compat updateProject`、`edit-project` 与 `delete-project` 也开始接受调用方直传 `projectUuid`，让 project editor general/project probes 的更新与删除路径复用现成 UUID，而不是每次再扫一遍项目列表；紧接着又把 `scripts/extract-assets` 这一组 caller 收口到纯 UUID-first，project editor / script editor / probe / workbench 在已有 `projectUuid` 时不再继续双传 `projectNumericId`，并补了请求体 helper 测试来锁住 “UUID 优先、numeric 仅 legacy fallback” 语义；最新一刀则把 `novels` workbench / events 的 UUID-path compat 签名也收成 UUID-first，让小说工作台、事件工作台与 compatibility probes 在已持有 `project.id` 的路径上不再继续传 `projectNumericId`，只给 legacy 调用保留 numeric fallback；这次再补到 task center 失败任务恢复链路：当 deep-link 元数据只带 `project_uuid`、没有 `project_numeric_id` 时，前端 parser 与产品壳导航也不会再直接丢掉上下文，而是继续把 `projectUuid + workspaceId + scriptNumericId` 同步进 workspace scope，保住 script / production 入口的 UUID-first 恢复能力；继续往前收后，global search 命中回产品壳时也不再要求 `project_numeric_id`，只要 metadata 已带 `project_id` UUID，就会把 `projectUuid + workspaceId (+ scriptNumericId)` 同步回 workspace/project scope，避免搜索命中重新退化成 numeric-only 恢复链路；顺手再把 novels compatibility probes 的 mutation caller 收成 explicit UUID path，项目编辑器 probe 在已持有 `projectUuid` 时改走 `deleteNovelByProjectUuid()` / `updateNovelByProjectUuid()`，不再继续借壳 scanningProjects 语义；最新这一刀则把 `system_probes` scope helper 里的 fake `1` fallback 继续拔掉，`resolveProductionProbeScope()` / `resolveAccountProbeClearScope()` 在缺真实 scope 时改为保留空值，production/settings/account probes 直接按 missing-scope `404` 记账，而不是再向伪造的 `projectId/scriptId = 1` 发请求；继续往前收后，task center 的项目筛选状态也开始保留 `projectUuid`，controller / workbench 在只有 UUID 的上下文里会先从当前已加载项目列表解析 `numericId` 再请求 `/jobs/page`，让产品壳 UUID-only restore path 能直接接到任务列表筛选；再补一格后，task center 的本地筛选与实时任务合并也开始同时识别 `project_uuid` / UUID-shaped `project_id` 与 `project_numeric_id`，避免 live update 回流重新退化成 numeric-only 过滤；最新一刀则把产品壳自己的 scoped project cache 也补成 UUID-first 清理语义，task center / global search / `/product/projects` 通知链接在只带 `projectUuid` 时会显式清空 `_productScopedProjectNumericId`，不再残留上一份 numeric scope；继续补齐后，task center workbench 的筛选 UI 本身也正式露出 `projectUuid` 输入框，和内部已存在的 UUID filter/controller 对齐，避免 UUID-only scoped 查询只能靠隐藏状态工作；这轮再把同类 restore 补到 `quality_reviews`，quality pane 现在会接收产品壳 `projectUuid`，并在打开 workbench / 读取 dashboard 前按需用 `/api/v1/projects` 解析 numeric id，避免 UUID-only 项目上下文一进质量面板就丢种子；再补一格后，quality workbench 顶部也会直接回显 scope seed，让当前 `projectId` 是原生 numeric 还是由 UUID restore 懒解析而来不再是黑箱；这一轮则把产品壳顶部 workspace context 的当前项目摘要也补成 UUID-aware，numeric 还没回填时也会优先按 `projectUuid` 识别项目，至少显示项目名或原始 UUID；这次继续把 restore 语义补到 `short_video_space` 内部 selector，UUID-only deep link / 搜索跳转把产品壳 scope 带进来后，space 自己也会按 `initialProjectUuid` 命中对应项目，而不是 reload 项目列表后偷偷漂回第一项；这一轮再把 notifications 到 product shell 的 scope restore 也补齐，`/product/jobs` 这类 linkPath 本身不带 scope 的通知现在会回退使用 `NotificationRecordV1` 和 payload 里的 `projectId/projectNumericId/workspaceId`，点开后不再只是进面板，还会同步恢复当前项目上下文；继续顺手补后，jobs pane 自己在选中列表项或按 jobId 查询详情成功后，也会从 `JobRow.payload` 回填 `project_uuid/project_numeric_id/workspace_id/script_*` 到产品壳，不再把它当成一条纯文本状态记录；这轮最后再把 `WorkspaceInputController` 的默认 `projectId/scriptId = 1` 拔掉，工作区输入在未显式选项目前保持空 scope，避免 fake `1` 再从 agent workspace / probe / writeback 入口回流成隐形上下文；继续往 storyboard 里收后，`production/storyboard/preview`、`update-url`、`remove-frame`、`update-live-action-reference` 与 `get-storyboard-data` 这几条 REST 也补上了 `projectUuid` 输入，storyboard editor / workbench 在已持有项目 UUID 时会优先把它透传给 production get-data / generate-data / track / prompt / video / voiceover 这一串调用，不再在 UI 层先退回 numeric-only；再往外围补一圈后，`script_editor/storyboards` 的 production summary、单条/批量新增分镜、batch workbench 的出图/预览/下载/导出，以及 storyboard media op 的 `selectVideo`、`enqueueVideoExport`、`patchRegeneration` 也都收成 UUID-first，只有 legacy 路径才继续双传 numeric 项目号；这一轮再把 `production/workbench/video_selection.dart` 底层 wrapper 与 `script_editor/edit_image` 入口也接上 UUID-first，至少 delete/select/batch* video 操作和 edit-image upload/generate 已经不再被 `required int projectId` 卡死；继续往使用面推进后，`short_video_space` 的 assembly clip desk、draft apply、batch operations、undo/redo，以及 typed production probe suite 也都开始把 `ProjectRow.id` 传进 `select-video`、`delete-video`、`update-duration` 和相关 storyboard/edit-image probe caller，终于不再明明拿着 UUID 还只发 numeric-only 请求；最新再补的这一格，则把 `short_video_space/section_production.dart` 的 batch candidate clips 与 production compare rows 拉取，以及 production probe 自己的 batch-candidate caller，也都补成 UUID-first，避免 project overview / probe 层又把上面已经拿到的 UUID 白白丢回 numeric-only；这一轮继续把底层 helper 收口到 `rust_api/production` 本身：`routes.dart` 现在有统一的 `buildProductionProjectScopeBodyV1()`，flow / generate-video / export / batch-candidate / polling 这些 endpoint 不再各写一份 UUID-first 分支；`storyboard/data.dart` 早期保留下来的 add / batch-add / edit / get-data 也改成统一复用 `buildStoryboardProjectScopeBodyV1()`，后面再清理 numeric 兼容时，至少先少掉一层“同样逻辑散落多处”的维护噪音；最新这一轮再把共享 helper 正式抽成 [`production/project_scope.dart`](../../frontend/lib/rust_api/production/project_scope.dart)，`assets/api.dart`、`edit_image/api.dart` 与 `workbench/tracks.dart` 这些 production 子域 caller 都开始走同一份 UUID-first 规则，底层分支继续显著变薄；继续再补一刀后，`workbench/generate.dart` 的 `generate-video-prompt`、`get-generate-data` 与 `generate-voiceover` 也都改成复用同一份 shared helper，production workbench 常用查询/生成入口的 scope 逻辑终于不再到处手搓。
