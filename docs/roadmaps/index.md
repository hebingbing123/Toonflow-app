...
竖切落地进度（Phase 1 执行节奏）：[`openflow-platform-progress.md`](./openflow-platform-progress.md)。
...
| [`workspace-release-checklist.md`](./workspace-release-checklist.md) | Workspace staging / 发布前检查单（串联 W9.2 / W9.3 / W9.4 / W10.3） |
...
1. **周计划**：从 [`openflow-platform-progress.md`](./openflow-platform-progress.md) 看当前 Phase；从本索引跳进对应方向文件，勾选 `next` 行。
...
4. **落到迭代**：把某个 WP 标成「本周在做」，做完把对应表格行从 `next` 改成 `baseline_done`（或打上完成日期），并在 [`openflow-platform-progress.md`](./openflow-platform-progress.md) 记一笔竖切 commit。
