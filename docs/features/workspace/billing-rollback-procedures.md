...
aws ssm put-parameter \
  --name /openflow/prod/billing/dual-write-enabled \
  --value "false" \
  --overwrite
...
curl -H "Authorization: Bearer $TOKEN" \
  'https://api.openflow.com/api/v1/me?v=2' | jq '.'
```
...
curl -X POST https://api.openflow.com/api/v1/jobs \
...
kubectl logs -l app=openflow-backend --tail=100 | grep -i "workspace.*billing.*disabled"

# Check for quota enforcement using user-scope
kubectl logs -l app=openflow-backend --tail=100 | grep -i "quota.*user.*scope"

# Check for webhook processing (should only update user profile)
kubectl logs -l app=openflow-backend --tail=100 | grep -i "webhook.*user_profile"
```
...
Questions? Contact support@openflow.com
...
|------|---------|---------|----------------|
| `OPENFLOW_BILLING_DUAL_WRITE_ENABLED` | Enable webhook dual-write | `false` | `false` |
| `OPENFLOW_BILLING_WORKSPACE_SCOPE_ENABLED` | Enable workspace quota enforcement | `false` | `false` |
| `OPENFLOW_ME_V2_ENABLED` | Enable `/me` v2 endpoint | `false` | `false` |
