...
本 spec 解决的不是“做一个完整 NLE”，而是把 Openflow 当前已经存在的分镜、候选视频、字幕/旁白文案、导出 sidecar，收束成一个真正可用的“成片装配 + 轻量粗剪”工作台。
...
虽然 `Jellyfish` 开源且值得借鉴，但 Openflow 当前更适合先做轻量剪辑台，而不是直接照搬完整剪辑器。
...
[`backend/src/production/workbench/storyboard_ops/export/zip_export.rs`](../../backend/src/production/workbench/storyboard_ops/export/zip_export.rs) 已经产出：
...
[`frontend/lib/storyboard_editor/video_section.dart`](../../frontend/lib/storyboard_editor/video_section.dart) 已经存在：
...
- 系统设置里存在 `ttsDubbing`，但当前是 disabled，见 [`backend/src/settings/agent_deploy/storage.rs`](../../backend/src/settings/agent_deploy/storage.rs)
