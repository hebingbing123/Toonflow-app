...
全局搜索功能为 Openflow 平台提供跨项目、剧本、资产的统一搜索能力。用户可通过主导航栏的全局搜索框快速查找内容，系统使用 PostgreSQL tsvector + GIN 索引实现中英文分词和权重排序，并在统一结果页按类型分组展示搜索结果。
...
```bash
curl -X GET "https://api.openflow.com/api/v1/search?q=角色设计&result_type=project&page=1&page_size=20" \
...
```bash
curl -X GET "https://api.openflow.com/api/v1/search/history" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```bash
curl -X DELETE "https://api.openflow.com/api/v1/search/history" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```dart
import 'package:openflow/global_search/global_search_bar.dart';
...
```dart
import 'package:openflow/global_search/search_results_page.dart';
...
```dart
import 'package:openflow/global_search/search_result_card.dart';
...
```dart
import 'package:openflow/global_search/advanced_filter_panel.dart';
...
```dart
import 'package:openflow/global_search/search_history_list.dart';
...
```bash
# 搜索包含"角色"的所有内容
curl -X GET "https://api.openflow.com/api/v1/search?q=角色" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```bash
# 仅搜索项目
curl -X GET "https://api.openflow.com/api/v1/search?q=角色&result_type=project" \
  -H "Authorization: Bearer YOUR_TOKEN"

# 搜索项目和剧本
curl -X GET "https://api.openflow.com/api/v1/search?q=角色&result_type=project&result_type=script" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```bash
# 搜索最近 30 天创建的内容
curl -X GET "https://api.openflow.com/api/v1/search?q=角色&time_from=2024-02-20T00:00:00Z" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```bash
# 获取第 2 页，每页 50 条
curl -X GET "https://api.openflow.com/api/v1/search?q=角色&page=2&page_size=50" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
- [Openflow API 文档](https://api.openflow.com/docs)
...
如有问题或建议，请联系开发团队：
- Email: dev@openflow.com
- GitHub Issues: https://github.com/openflow/openflow-app/issues
