...
Toonflow can persist **large export artifacts** to **S3-compatible object storage** instead of local disk so that **multiple API replicas** can enqueue workers, serve downloads, and run migrations without relying on a shared filesystem.
...
|--------|----------------|--------------|------------------------|
| Workspace shared cleared-template audit export (async) | `settings.workspace_shared_audit.export` | `TOONFLOW_WORKSPACE_SHARED_AUDIT_EXPORT_S3_BUCKET` | `workspace-shared-audit-exports` |
| Account data export (zip) | `settings.account.export` | `TOONFLOW_ACCOUNT_EXPORT_S3_BUCKET` | `account-exports` |
...
- You run **more than one** `toonflow-server` / worker pod without a **ReadWriteMany** volume for export directories.
...
If **no** bucket env vars are set, behavior stays **local**: `TOONFLOW_LOCAL_*` dirs or OS temp (see `backend/src/state/from_env.rs` and account/workspace export modules).
...
| `TOONFLOW_EXPORT_S3_ENDPOINT` | Optional | **Preferred** single URL for MinIO or a custom S3 endpoint. First non-empty among the three endpoint vars wins for client configuration. |
| `TOONFLOW_EXPORT_S3_FORCE_PATH_STYLE` | Optional | `1` / `true` forces path-style addressing (typical for MinIO). If you set any custom endpoint and do **not** set any `*_FORCE_PATH_STYLE` to `false`, the server defaults path-style to **true**. |

Per-feature endpoint overrides (used only if `TOONFLOW_EXPORT_S3_ENDPOINT` is empty):
...
|----------|-------------------|-------------|
| `TOONFLOW_WORKSPACE_SHARED_AUDIT_EXPORT_S3_BUCKET` | **Yes** to enable S3 | Target bucket name. |
| `TOONFLOW_WORKSPACE_SHARED_AUDIT_EXPORT_S3_PREFIX` | No | Key prefix; default `workspace-shared-audit-exports`. No leading/trailing slashes required. |

Local fallback when S3 is **not** enabled:

- `TOONFLOW_LOCAL_WORKSPACE_SHARED_AUDIT_EXPORT_DIR` (optional; otherwise temp under `toonflow-workspace-shared-audit-exports`).

### Account export zip

| Variable | Required for S3 | Description |
|----------|-----------------|-------------|
| `TOONFLOW_ACCOUNT_EXPORT_S3_BUCKET` | **Yes** to enable S3 | Target bucket name. |
| `TOONFLOW_ACCOUNT_EXPORT_S3_PREFIX` | No | Key prefix; default `account-exports`. |

Local fallback:

- `TOONFLOW_LOCAL_ACCOUNT_EXPORT_DIR` (optional; otherwise temp).
...
- `account-exports/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee/toonflow-account-export-aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee-20260511T120000Z.zip`
...
3. Set `TOONFLOW_EXPORT_S3_ENDPOINT` to your MinIO API base URL (e.g. `https://minio.example.com`) and `TOONFLOW_EXPORT_S3_FORCE_PATH_STYLE=true` if you use path-style.
...
- `TOONFLOW_EXPORT_S3_ENDPOINT` set (shared override).
...
```bash
aws s3 ls "s3://$TOONFLOW_WORKSPACE_SHARED_AUDIT_EXPORT_S3_BUCKET/workspace-shared-audit-exports/$USER_UUID/" --endpoint-url "$TOONFLOW_EXPORT_S3_ENDPOINT"
```
...
1. Objects under **`{TOONFLOW_ACCOUNT_EXPORT_S3_PREFIX}/{user_id}/`** in the account export bucket (if account S3 is enabled).
2. Objects under **`{TOONFLOW_WORKSPACE_SHARED_AUDIT_EXPORT_S3_PREFIX}/{user_id}/`** in the workspace shared audit bucket (if that bucket is enabled).

Local directories under `TOONFLOW_LOCAL_*` are still removed as before. Failures in S3 cleanup are **non-fatal** (logged only via `let _ =`); add bucket lifecycle rules as a safety net.
...
| MinIO `SignatureDoesNotMatch` | Clock skew or wrong endpoint / path-style | Sync NTP; try `TOONFLOW_EXPORT_S3_FORCE_PATH_STYLE=true`. |
