...
- **W4.5 已起步**：`app_generation_job` 先按 `project_id` 派生 workspace 可见范围，`GET /api/v1/jobs/page`、`GET /api/v1/jobs`、jobs 三个 summary 端点，以及 jobs 详情/取消/重试链路已统一到 owner 或同 workspace 项目成员可见（未传 `project_id` 的列表/汇总仍保留 owner 个人视图）；`POST /api/v1/jobs` 在 payload 带 project 字段时会做成员校验并规范化 `project_uuid`/`project_numeric_id`
