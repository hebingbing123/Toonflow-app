...
- **最新补齐**：内容与合规队列 **P-C2** 已补到 reviewer ownership / SLA drilldown — `GET /api/v1/internal/compliance/reports` 现同步返回 reviewer/owner 负载汇总，并支持按 `claimedByLabel` / `slaBucket` 过滤当前队列；Flutter `ContentComplianceSection` 已把 `open>24h` / `open>72h` / `claimed>24h` / `critical未claim` 摘要变成可点击 drilldown，同时补 reviewer 负载卡片，运营开始能看出“谁手里压着单、哪类 breach 需要先清”
...
- **下一条平台治理竖切候选**：继续把 **P-C2** 往 reviewer reassignment / SLA 导出推进，或回到 **P-A2** 做服务端保存视图与跨端同步
