...
- **W4.5 已起步**：`app_generation_job` 先按 `project_id` 派生 workspace 可见范围，`GET /api/v1/jobs/page` 与 `GET /api/v1/jobs` 在传 `project_id` 时改为 workspace 成员可见项目内任务（未传 `project_id` 维持 owner 个人视图）
