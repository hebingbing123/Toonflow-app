...
- **最新补齐**：管理台扩展 **P-C1** 已继续补到 project remediation — 在既有 internal-only REST `GET /api/v1/internal/admin/search`、`GET /api/v1/internal/admin/users/{user_id}`、`GET /api/v1/internal/admin/workspaces/{workspace_id}`、`GET /api/v1/internal/admin/projects/{project_id}`、`POST /api/v1/internal/admin/users/{user_id}/governance`、`POST /api/v1/internal/admin/users/{user_id}/workspace-context`、`POST /api/v1/internal/admin/workspaces/{workspace_id}/governance` 之外，再新增 `POST /api/v1/internal/admin/projects/{project_id}/governance`，继续统一复用 `TOONFLOW_INTERNAL_OPS_TOKEN` / `X-Toonflow-Internal-Token` 门禁；后端支持把项目迁回 owner 仍具成员关系且未归档的目标 workspace，并自动清理失效/冗余 `app_project_member` ACL、同步回写关联 jobs payload `workspace_id`，同时新增 `app_project_governance_audit` 记录 remediation 轨迹；Flutter internal build 下的「管理台」pane 现已支持 project workspace 归属修复、项目 ACL 列表与 project 治理审计查看
...
- **下一条平台治理竖切候选**：在 **P-C1** 同一能力族内继续补 project owner transfer / workspace owner transfer 的 internal ops 补救入口，把当前“能查 + 能修 scope / 成员关系”扩到“能处理 ownership 异常”
