...
- **最新补齐**：Workspace 项目级 ACL **W5.2** 已补后端基线 — 新增 `app_project_member`，项目开始配置显式 ACL 后普通 member 会按 `viewer` / `editor` 收敛到读写权限；同时补了 `/api/v1/projects/{project_id}/members*` 管理端点、项目列表/摘要可见性过滤，以及 publish 写接口的 editor 门禁
