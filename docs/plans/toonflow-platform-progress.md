...
- **最新补齐**：内容与合规队列 **P-C2** 已从单条审核扩到队列治理视图 — `GET /api/v1/internal/compliance/reports` 现同步返回 SLA 摘要与 workspace 热点汇总，新增 internal-only `POST /api/v1/internal/compliance/reports/batch-mutate` 支持批量 `claim|resolve|dismiss`；Flutter `ContentComplianceSection` 已补 workspace 热点筛选、选中开放项后的批量处理条，以及原有 audit 时间线弹窗，运营不再只能逐条点单据
...
- **下一条平台治理竖切候选**：继续把 **P-C2** 往 reviewer ownership / SLA breach drilldown 推进，或回到 **P-A2** 做服务端保存视图与跨端同步
