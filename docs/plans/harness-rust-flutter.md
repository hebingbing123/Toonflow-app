...
todos:
  ...
  - id: jobs-and-webhook-hardening
    content: 长时生成：**进度**：`app_generation_job` + REST + **本机 worker** + **WS**；**queued/running 取消**、`failed` **重试入队**、`Idempotency-Key` 去重；**每 IP 429 + Retry-After**（健康检查路由除外）；**仍缺**独立队列、分布式限流；计费 webhook 验签与去重（§13）
    status: pending
isProject: false
---
