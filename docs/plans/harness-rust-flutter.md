...
todos:
  ...
  - id: master-detailed-parity-audit
    content: 基于 **`master`** 做逐项详细对比（旧 **`src/routes/**`**、**`src/socket/**`**、**`src/agents/**`**、前端调用面与当前 **`backend/`**、**`frontend/`**、OpenAPI、WS 文档逐一核对），把“接口已迁移”“设计性替代”“真实遗漏”“前端仅 probe 未完成产品 UX”的边界重新厘清；审计结论见 **[`master-detailed-parity-audit.md`](./master-detailed-parity-audit.md)**。当前确认的明确缺口至少包括：**`POST /api/production/editImage/uploadImage`**、**`POST /api/script/batchAddScript`**、旧 **script/production Socket.IO Agent** 的域工具与子 Agent 编排尚未迁入 Harness、Flutter 仍以 probe/debug shell 为主而非完整产品信息架构。
    status: pending
