...
5. **刻意保留（直至迁移窗口结束）**：**`toonflow-sqlite-import`**（`backend/src/bin/sqlite_import.rs`）等 **SQLite → Postgres** 工具可暂留；若不再执行迁移，再移至 **`scripts/`** 或单独 repo 并更新文档。
