...
todos:
  ...
  - id: master-detailed-parity-audit
    content: 基于 **`master`** 做逐项详细对比（旧 **`src/routes/**`**、**`src/socket/**`**、**`src/agents/**`**、前端调用面与当前 **`backend/`**、**`frontend/`**、OpenAPI、WS 文档逐一核对），把“接口已迁移”“设计性替代”“真实遗漏”“前端仅 probe 未完成产品 UX”的边界重新厘清；审计结论见 **[`master-detailed-parity-audit.md`](./master-detailed-parity-audit.md)**。已收口明确 REST 漏项（`production/edit-image/upload-image`、`scripts/batch-add`）、script 侧核心只读域工具（`get_planData`、`get_script_content`、`get_novel_text`、`get_novel_events`）以及 production 侧核心域工具（`get_flowData`、`add_deriveAsset`、`del_deriveAsset`、`generate_deriveAsset`、`generate_storyboard`）；当前主要缺口集中在 script/production 子 Agent 编排，以及 Flutter 仍以 probe/debug shell 为主而非完整产品信息架构。
    status: pending
