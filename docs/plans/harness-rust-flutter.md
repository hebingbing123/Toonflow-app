...
todos:
  ...
  - id: supabase-auth
    ...
    status: completed
  - id: flutter-shell
    content: Flutter 桌面+Web 均以可配置 baseUrl 连接 Rust；默认端口 8666；dev 例 http://127.0.0.1:8666，prod 为部署 URL；CORS/WebSocket/鉴权与后端一致；**竖切**：首页调试区已接项目/剧本/分镜 REST、**jobs** 列表/**`GET /api/v1/jobs/{id}`**/**`GET jobs?kind=flutter.probe`**/**`GET jobs?status=failed`**/**`GET jobs?kind=flutter.probe&status=queued`**/**`GET /api/v1/jobs/kinds`**/**`kinds/summary`**/**`status/summary`**/探针创建，与 **`GET /api/v1/skills/summary`** / **skills** / **harness/tools** 只读探针、**WS** `harness.tool.invoke` / **`harness.agent.run`**（Flutter 探针：attach + 多轮工具；**echo**、**isolated.echo**、**skills.read**、**wasm.probe**，与 Skill path 联动）；新增 **`agent_workspaces_section`** 作为 script/production 工作台入口（attach + `harness.agent.run` + `get_flowData` probe 收口），其中 production 工作区已把最新工具结果继续收口为 flow 摘要与“下一步建议”卡片，可按 `assets` / `scriptPlan` / `storyboardTable` / `storyboard` 状态自动推荐读取 flow、切换子代理与填充提示词，并支持在建议卡上一键读取 flow 或运行子代理；本轮再新增固定的 `scriptPlan → assets → storyboardTable → storyboard` 阶段看板，按空白/待补图/建议刷新/已完成状态显示当前制作环节，并可直接推进对应阶段动作；script 工作区也同步新增固定的 `storySkeleton → adaptationStrategy → novel context → script content` 阶段看板，按待生成/待读取/已就绪/已完成状态显示当前改编环节，并可直接读取上下文或推进对应子代理；调试区含 **`/health`**、**`/api/v1/health`**、**`/api/v1/ping`**、**`/api/v1/version`**、**`/api/v1/ready`**、**`/api/v1/usage/summary`**、**`GET /api/v1/models`** / **`models/detail`**（静态目录）、**`agents/memory/query`**（首项目 legacy id）；**`POST /api/v1/projects`** 空项目探针；Projects 页除 **`GET …/projects/summary`** 汇总与 **`GET …/art-styles`** 列表外，现已新增“画风工作台”，可统一执行 **`GET/POST/PATCH/DELETE …/art-styles*`**、读取 **`GET …/art-styles/legacy/{id}/cover`** 本地封面并调用 **`POST …/art-styles/extract-prompt`** 生成 prompt；同时也已新增“创作手册工作台”，统一承载导演手册与视觉手册的刷新、查看、创建、更新、删除，并用 `GET/POST /api/v1/visual-manual` 与 legacy project 手册写接口形成首页产品入口；Projects 主区还新增了“记忆工作台”，可按项目 legacy id、agent type 与可选 episodes id 查询、追加、清理 Agent 记忆，旧首页首项目 memory probe 下沉到兼容性折叠区；“质量评审”主区也已升级为正式工作台入口，可在一个对话框中完成评审筛选、坏例查看、统计/阶段通过率读取、详情查询与手动创建，旧 probe 创建入口下沉到兼容性折叠区；“任务中心”主区现也已升级为正式工作台入口，可在一个对话框中统一完成任务项目/分类读取、按项目/分类/状态筛选列表，以及按 legacy id 或 UUID 打开详情，旧加载与详情 probe 下沉到兼容性折叠区；项目详情对话框展示 **`GET …/projects/legacy/{id}/stats`**、**`GET …/novels`**（**`fetchProjectNovelsByLegacyId`** 与 **POST/GET/PATCH/DELETE** 探针），其中“小说与事件”主视图已进一步精简为章节/事件正式工作台摘要卡，旧 REST 首条/末条探针退出主链路，仅在兼容性折叠区保留回归入口；章节工作台已把 `get-novel-data` / `get-novel-index` / `get-novel-event-state` / `batch-delete` 等 legacy 包装读写收口为正式批量动作；**`GET …/assets`**（**`fetchProjectAssetsByLegacyId`** / **`ListAssetsResponse`**；**`page`/`limit`** 分页探针；**`asset_type`/`name`** 筛选探针；**`script_legacy_id`+`page`/`limit`** 与下拉联动探针）、**`GET …/assets/{aid}`**（**`fetchProjectAssetByLegacyIds`**）与刷新按钮，及资产正式工作流表单（显式目标选择的 **POST/PATCH/DELETE**、**PUT/DELETE** 剧本–资产关联与高级筛选），并新增“资产历史图工作台”用于 `corner-scape` 资产查询与历史图预览、“上传 Clip 资产”正式表单用于 `upload-clip` 入库、“资产图片工作台”用于 `GET/POST/PATCH/DELETE …/assets/{aid}/images*` 与预览，以及“资产出图工作台”用于 `production/assets/get-assets-data` 摘要、legacy `get-material-data` / `batch-generation-data` / `polling-prompt-assets`、`batch-generate-assets-image`、`polling-image`、`delete-assets-derivative` 与单资产 URL 更新；原首条/末条资产动作保留在兼容性折叠区；项目剧本区已新增“剧本批量工作台”，可按名称读取 `get-script-api` 上下文、显式选择目标剧本 id，并在同一入口内完成批量导出、提取状态轮询、素材抽取与批量创建；剧本编辑对话框除 **`DELETE`** 外，已升级为 script/workbench 正式入口，可直接读取当前脚本上下文、查看关联素材摘要、导出 ZIP、轮询提取状态并发起素材抽取，并新增“编辑图片工作台”用于加载 edit-image flow 模板与默认模型、上传源图、保存/更新 flow 步骤状态以及发起 `generate-flow-image`；分镜列表对话框已新增“分镜出图工作台”，可在剧本上下文内批量勾选分镜、沿用现有提示词发起 `batch-generate-image`、读取当前预览图与下载链接并导出所选分镜 ZIP；并在增删剧本后尝试刷新 stats；可 **`POST 空剧本`**、**`DELETE`** 项目；分镜列表对话框可 **`POST 空分镜`**；分镜编辑对话框除 **`DELETE`** 外，已升级为 storyboard/workbench 正式入口，可直接读取/保存预览图、管理视频轨道、生成默认视频提示词、提交视频任务并切换当前候选视频
    status: completed
  - id: master-detailed-parity-audit
    content: 基于 **`master`** 做逐项详细对比（旧 **`src/routes/**`**、**`src/socket/**`**、**`src/agents/**`**、前端调用面与当前 **`backend/`**、**`frontend/`**、OpenAPI、WS 文档逐一核对），把“接口已迁移”“设计性替代”“真实遗漏”“前端仅 probe 未完成产品 UX”的边界重新厘清；审计结论见 **[`master-detailed-parity-audit.md`](./master-detailed-parity-audit.md)**。已收口明确 REST 漏项（`production/edit-image/upload-image`、`scripts/batch-add`）、script 侧核心只读域工具（`get_planData`、`get_script_content`、`get_novel_text`、`get_novel_events`）、production 侧核心域工具（`get_flowData`、`add_deriveAsset`、`del_deriveAsset`、`generate_deriveAsset`、`generate_storyboard`）以及 script/production 子 Agent 编排工具（`run_sub_agent_*` / `run_supervision_agent`）；Flutter `agent_workspaces_section` 已升级为 script/production 双工作区任务化入口（模板提示词、子 Agent 快捷选择、script/prod 域工具均支持 JSON 参数探测、WS 最近事件摘要），并补齐最小结果回写闭环（script 侧优先回写 `get_script_content.content`，并支持 `run_sub_agent_script` 结果直写候选，且支持基于 `get_planData` 一键回写 `set-plan-data`；同时 script 工作区现已根据 `get_planData` / `get_novel_text` / `get_novel_events` / `get_script_content` 自动生成下一步建议卡，直接读取上下文或运行对应子代理，并把 `get_novel_text` / `get_novel_events` 的原始 `items` 数据直接渲染为章节/事件快照，附带 `novelId` 一键填参；production 侧支持通用工具结果写回扩展 key，核心 key 在 `get_flowData` 直写外，还支持资产/分镜变更工具与 production 子 Agent 工具先刷新对应 flow key 再安全回写，并提供建议 key 快捷应用）；剧本编辑器已进一步收口为 script workbench 入口，可直接处理脚本上下文、导出、提取状态与素材抽取；剧本分镜编辑器已进一步收口为 production storyboard/video 工作台入口，可直接处理预览图、轨道、默认视频提示词、视频生成提交以及候选视频选择/删除，并把 workbench 逻辑拆为独立职责文件以控制单文件复杂度；并在 UI 形态上新增 Agent Workspace 二级导航（Script / Production / Activity）与执行活动面板，将双工作区卡片拆分为独立职责组件文件，减少单文件复杂度并提升可维护性。
    content: 基于 **`master`** 做逐项详细对比（旧 **`src/routes/**`**、**`src/socket/**`**、**`src/agents/**`**、前端调用面与当前 **`backend/`**、**`frontend/`**、OpenAPI、WS 文档逐一核对），把“接口已迁移”“设计性替代”“真实遗漏”“前端仅 probe 未完成产品 UX”的边界重新厘清；审计结论见 **[`master-detailed-parity-audit.md`](./master-detailed-parity-audit.md)**。已收口明确 REST 漏项（`production/edit-image/upload-image`、`scripts/batch-add`）、script 侧核心只读域工具（`get_planData`、`get_script_content`、`get_novel_text`、`get_novel_events`）、production 侧核心域工具（`get_flowData`、`add_deriveAsset`、`del_deriveAsset`、`generate_deriveAsset`、`generate_storyboard`）以及 script/production 子 Agent 编排工具（`run_sub_agent_*` / `run_supervision_agent`）；Flutter `agent_workspaces_section` 已升级为 script/production 双工作区任务化入口（模板提示词、子 Agent 快捷选择、script/prod 域工具均支持 JSON 参数探测、WS 最近事件摘要），并补齐最小结果回写闭环（script 侧优先回写 `get_script_content.content`，并支持 `run_sub_agent_script` 结果直写候选，且支持基于 `get_planData` 一键回写 `set-plan-data`；production 侧支持通用工具结果写回扩展 key，核心 key 在 `get_flowData` 直写外，还支持资产/分镜变更工具与 production 子 Agent 工具先刷新对应 flow key 再安全回写，并提供建议 key 快捷应用；读取 `assets` / `storyboard` flow 后还能直接从当前快照生成 `generate_deriveAsset` / `generate_storyboard` 的 `ids` 参数，以及 `add_deriveAsset` / `del_deriveAsset` 的资产树模板参数）；剧本编辑器已进一步收口为 script workbench 入口，可直接处理脚本上下文、导出、提取状态与素材抽取；剧本分镜编辑器已进一步收口为 production storyboard/video 工作台入口，可直接处理预览图、轨道、默认视频提示词、视频生成提交以及候选视频选择/删除，并把 workbench 逻辑拆为独立职责文件以控制单文件复杂度；并在 UI 形态上新增 Agent Workspace 二级导航（Script / Production / Activity）与执行活动面板，将双工作区卡片拆分为独立职责组件文件，减少单文件复杂度并提升可维护性。
    status: completed
