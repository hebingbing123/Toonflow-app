...
todos:
  ...
  - id: harness-rust-core
    content: 在 Rust 中落地 Harness 分层（工具/权限/观测/Agent 循环），替换 vm2 类沙箱为进程或 WASM 等硬隔离方案；**进度**：`ToolRegistry` 静态目录（工具 `name`+`description`）、`GET /api/v1/harness/tools`、只读 `GET /api/v1/skills*`
    status: pending
  - id: rust-backend-mvp
