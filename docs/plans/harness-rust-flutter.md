...
todos:
  ...
  - id: harness-rust-core
    content: 在 Rust 中落地 Harness 分层（工具/权限/观测/Agent 循环），替换 vm2 类沙箱为进程或 WASM 等硬隔离方案；**进度**：`ToolRegistry` 静态目录；**HTTP** **`GET /api/v1/harness/tools`** 在 **`backend/src/harness/http.rs`**（与只读 **`GET /api/v1/skills*`** 解耦）；**WS** 载荷类型在 **`harness/wire.rs`**；**WS** 分支逻辑在 **`harness/ws_tool.rs`** / **`harness/ws_agent.rs`**（`harness.agent.run` 参数聚合为 **`HarnessAgentWsParams`**）；**observe** 挂钩 **memory REST**、**job worker**（**claimed** / **succeeded** / **failed** / **cancelled**）；**`harness/ws_chat.rs`**（**`agent.chat.send`** 流式）；**`harness/ws_channel.rs`** + **`harness/ws_session.rs`**（attach / context.update）；**`permissions`**：**`HARNESS_WS_CHANNELS`** 限制 **`script`** / **`production`** attach；**`harness/wire.rs`** 含 session/chat attach 载荷类型；**WS** `harness.tool.invoke` / `harness.tool.result`（**`echo`**、**`isolated.echo`**（子进程 + **`HARNESS_ISOLATE_MAX_CONCURRENT`** 并发槽）、**`skills.read`**、**`wasm.probe`**（**wasmi** + `build.rs` 嵌入 WASM））、**WS** **`harness.agent.run`**（OpenAI 非流式 **tools** 多轮闭环 + 最终 `chat.message.*`）；文档 **`docs/websocket-events.md`** / OpenAPI 摘要已对齐。**仍缺**（产品化加深）：通用「用户上传 WASM」沙箱与策略、进程池预热/回收、与 LLM 流式工具调用融合、完整观测/trace 管线；**域功能**继续按竖切迁入 Rust 并挂 Harness 策略/观测
    status: completed
  - id: rust-backend-mvp
    content: Rust 后端 MVP：**主库仅为 Supabase Postgres**（§4.1；SQLx 直连）；旧 SQLite 仅迁移源；AI Provider 流式；**竖切**：Flutter 已接项目/剧本/分镜 REST（**`POST` 新建项目**、**`POST` 项目下新建剧本**、**`POST` 剧本下新建分镜**、列表+按 legacy `GET/PATCH`、**`DELETE` 项目/剧本/分镜**）；**项目统计**：**`GET /api/v1/projects/summary`**（用户维度项目/剧本/分镜总数）、**`GET /api/v1/projects/legacy/{id}/stats`**（单项目；角色/视频占位 0，对齐旧 **`generalStatistics`**）；**静态模型目录**：**`GET /api/v1/models`** / **`GET /api/v1/models/detail`**（`data/models_catalog.json` 编译嵌入；旧 **`modelSelect`** 过滤语义）；**任务**：`app_generation_job` + REST（**`GET /api/v1/jobs`** 可选 **`kind`**/**`status`** query）+ **进程内 worker**（**PG + SKIP LOCKED** 已可多实例）、取消/重试/幂等、**WS** `generation.job.updated`；**GET /api/v1/me** 含 **`plan_tier`**；**用量**：`app_usage_event` + **`GET /api/v1/usage/summary`**（总量 + **`event_counts_last_7d`**）；**Agent 记忆**：`app_agent_memory` + **`POST /api/v1/agents/memory/query|clear|append`**（旧 getMemory/clearMemory 语义；append 无自动摘要）；**可观测**：`X-Request-Id` + 错误 JSON `request_id`；**`GET /api/v1/version`** 可选编译期 **`git_sha`**（**`TOONFLOW_GIT_SHA`**）；**契约**：OpenAPI/WS 与集成测试（isolate）；**进度**：**`backend/src/app.rs`** 内 **无 PG 契约烟雾单测**（**`/health`**、**`/api/v1/health`**、**`/api/v1/version`**、**`/api/v1/ready`**；**Supabase 形 HS256 JWT**：**`/api/v1/models`**、**`/api/v1/models/detail`**、**`/api/v1/harness/tools`**、**`/api/v1/skills/summary`**、**`/api/v1/skills`**、**`/api/v1/skills/content`**）；**`app::pg_contract_tests`**（**`#[ignore]`**，需 **`DATABASE_URL`** + **`SUPABASE_JWT_SECRET`** + 已迁移库：**`POST /api/v1/projects`** → **`GET …/stats`** → **`DELETE …`** → 404）；**不依赖 Redis 才能完成重构**；仍缺更广的端到端契约回归矩阵与业务域全覆盖
    status: pending
  - id: postgres-ops
