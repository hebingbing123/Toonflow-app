...
todos:
  ...
  - id: harness-rust-core
    content: 在 Rust 中落地 Harness 分层（工具/权限/观测/Agent 循环），替换 vm2 类沙箱为进程或 WASM 等硬隔离方案；**进度**：`ToolRegistry` 静态目录；**HTTP** **`GET /api/v1/harness/tools`** 在 **`backend/src/harness/http.rs`**（与只读 **`GET /api/v1/skills*`** 解耦）；**WS** 载荷类型在 **`harness/wire.rs`**；**WS** 分支逻辑在 **`harness/ws_tool.rs`** / **`harness/ws_agent.rs`**（`harness.agent.run` 参数聚合为 **`HarnessAgentWsParams`**）；**observe** 挂钩 **memory REST**、**job worker**（**claimed** / **succeeded** / **failed** / **cancelled**）；**`harness/ws_chat.rs`**（**`agent.chat.send`** 流式）；**`permissions`**：**`HARNESS_WS_CHANNELS`** 限制 **`script`** / **`production`** attach；**`harness/wire.rs`** 含 session/chat attach 载荷类型；**WS** `harness.tool.invoke` / `harness.tool.result`（**`echo`**、**`isolated.echo`**（子进程 + **`HARNESS_ISOLATE_MAX_CONCURRENT`** 并发槽）、**`skills.read`**、**`wasm.probe`**（**wasmi** + `build.rs` 嵌入 WASM））、**WS** **`harness.agent.run`**（OpenAI 非流式 **tools** 多轮闭环 + 最终 `chat.message.*`）；文档 **`docs/websocket-events.md`** / OpenAPI 摘要已对齐。**仍缺**（产品化加深）：通用「用户上传 WASM」沙箱与策略、进程池预热/回收、与 LLM 流式工具调用融合、完整观测/trace 管线；**域功能**继续按竖切迁入 Rust 并挂 Harness 策略/观测
    status: completed
  - id: rust-backend-mvp
