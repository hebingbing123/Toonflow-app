...
todos:
  ...
  - id: harness-rust-core
    ...
    status: pending
  - id: rust-backend-mvp
    content: Rust 后端 MVP：**主库仅为 Supabase Postgres**（§4.1；SQLx 直连）；旧 SQLite 仅迁移源；AI Provider 流式；**竖切**：Flutter 已接项目/剧本/分镜 REST（分镜列表+按 legacy `GET/PATCH`）；**任务**：`app_generation_job` + REST + **进程内 worker**（`FOR UPDATE SKIP LOCKED`；`flutter.probe` 等）；**WS** `generation.job.updated`；仍缺分布式队列、取消/重试、Harness 硬隔离与契约测试固化
    status: pending
  - id: postgres-ops
  ...
  - id: jobs-and-webhook-hardening
    content: 长时生成：**进度**：`app_generation_job` + REST + **本机 worker** + **WS**；**queued/running 取消**、`failed` **重试入队**、`Idempotency-Key` 去重；**每 IP 429 + Retry-After**（健康检查除外；可选受信代理下 `Forwarded`/`X-Forwarded-For`）；**仍缺**独立队列、分布式协调限流；计费 webhook 验签与去重（§13）
    status: pending
isProject: false
---
