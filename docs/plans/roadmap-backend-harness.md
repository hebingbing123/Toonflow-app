...
| 项 | 内容 |
|----|------|
| **目标** | `GET …/projects/summary` 与 `GET …/projects/{id}/stats` 中视频计数与产品定义一致（或为明确占位 Document）。 |
| **依赖** | 产品确认「视频」判定（分镜导出 / job 成功态 / 存储对象存在）。 |
| **PR 切片** | （1）schema 或查询定义；（2）handler + OpenAPI 字段说明；（3）Flutter 展示文案（若有）。 |
| **触点** | 项目 stats 相关 handler（`backend/src/app/` 下 projects）；OpenAPI；`frontend/lib/rust_api/` 若暴露该字段。 |
| **测试** | 扩展 PG 契约：插入 fixture job/asset 后期望计数；或单元测试查询层。 |
| **回滚** | 恢复占位 `0` 与文档说明。 |
...
| **测试** | 单元：畸形 WASM、超时；契约：拒绝未授权上传；可选 fuzz 体量上限。 |
...
| **PR 切片** | （1）指标：队列深度、等待时间、进程复用次数；（2）可选进程池或 worker 常驻；（3）配置项与默认值文档化。 |
| **触点** | `backend/src/harness/isolate.rs`；`backend/src/harness/observe.rs`；环境变量 README。 |
| **测试** | 负载脚本或 bench（非 CI 必选）；单测模拟并发槽耗尽。 |
| **回滚** | 配置关闭预热，回退代码路径到「按需启动」。 |
...
| **依赖** | 当前 LLM envelope 与 `harness.agent.run` 协议；产品设计（是否允许并行工具）。 |
...
### WP-F：Trace / 结构化观测（可选 OTel）

| 项 | 内容 |
|----|------|
| **目标** | `request_id` 基础上，把 harness session / tool invoke 串成可查询轨迹（日志或 OTel spans）。 |
| **依赖** | 与 [`roadmap-quality.md`](./roadmap-quality.md) WP-D 的关联字段设计对齐（可先只做 trace id 传递）。 |
| **PR 切片** | （1）统一 `tracing` span 字段；（2）导出配置（feature gate）；（3）采样率与 PII 脱敏策略。 |
| **触点** | `backend/src/harness/observe.rs`；HTTP 中间件；`jobs/worker` 若需关联 job id。 |
| **测试** | 本地 Jaeger/OTel collector 冒烟；单测 span 父子关系。 |
| **回滚** | 关闭导出 feature，仅保留日志文本。 |
