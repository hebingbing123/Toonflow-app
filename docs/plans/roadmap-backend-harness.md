...
| 内容 | 状态 | 备注 |
|------|------|------|
| 用户上传 WASM 策略（配额、签名、审计） | `in_progress` | **薄切片**：`wasm_runtime::validate_user_wasm_upload` + **`HARNESS_USER_WASM_MAX_BYTES`** + **`POST /api/v1/harness/user-wasm/validate`**（JWT）+ Postgres Stub **`POST/GET /api/v1/harness/user-wasm`**（`app_harness_user_wasm`；列表不返回 `wasm_bytes`）+ 单测 / 冒烟；**吊销（软删除）**已实现（设置 `revoked_at`、列表过滤/配额不计入）；对象存储 / WS 执行仍为 **`next`**；威胁模型见 [`harness-user-wasm-threat-model.md`](./harness-user-wasm-threat-model.md) |
