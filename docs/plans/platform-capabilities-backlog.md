...
| P-C2 | **内容与合规队列**（举报、人工审核） | 表 + 工作流 API | 审核台 UI | **tracked**：新增 `app_content_compliance_report`、`app_content_compliance_audit` 与 `POST /api/v1/content/reports`、`GET /api/v1/internal/compliance/reports`、`GET /api/v1/internal/compliance/reports/{id}/audit`、`POST /api/v1/internal/compliance/reports/{id}/claim`、`POST /api/v1/internal/compliance/reports/{id}/resolve`、`POST /api/v1/internal/compliance/reports/batch-mutate`、`POST /api/v1/internal/compliance/reports/reassign`；用户可按 `project|script|storyboard|asset|novel|user` 提交举报，服务端自动解析关联 `workspace_id` / `project_id` 并校验 workspace membership；internal ops 侧按 `pending -> claimed -> resolved|dismissed` 状态机处理，resolve 已支持 `none|archive_project|suspend_user` disposition 模板，并把 `reported / claimed / resolved|dismissed / disposition_applied / reassigned` 写入不可变 audit 时间线；`GET /api/v1/internal/compliance/reports` 现同步返回 SLA 摘要、workspace 热点与 reviewer/owner 负载汇总，并支持按 `claimedByLabel` / `slaBucket` 下钻过滤；Flutter `ContentComplianceSection` 作为独立 pane 已补举报入口、审核队列摘要、workspace 维度热点筛选、reviewer 负载卡片、SLA breach drilldown、批量 claim/resolve/dismiss、批量改派 reviewer、当前筛选结果 CSV 导出、target/report UUID 复制、audit 时间线弹窗，以及跳回项目详情 / 管理台上下文的联动入口 |
