...
| `/api/artStyle/*` | 画风库 CRUD / 抽 prompt | 🟡 | **`app_art_style` + REST**；**`promote_legacy_from_staging()`** 含 **`o_artStyle`→`app_art_style`**（**`owner_user_id`** 见迁移说明）；**`GET`/`POST /api/v1/art-styles`**、**`GET`/`PATCH`/`DELETE …/art-styles/legacy/{id}`**（RLS）；Flutter **`fetchArtStyles`** 探针；**无 DB 烟雾** → **503**；**`pg_contract`** **`art_styles_crud_roundtrip`** / **`promote_staging_*`**；旧 **base64→OSS 封面**、**`extractStylePrompt`**（多模态抽 prompt）仍 ⏳ |
