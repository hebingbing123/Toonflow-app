...
| `/api/production/**` | 分镜图/视频工作台、流、导出 | 🟡 | **Storyboard** 已有 **`app_storyboard` + REST**；**`POST /api/v1/production/get-production-data`**、**`get-flow-data`**、**`save-flow-data`**、**`workbench/generate-video`**、**`storyboard/polling-image`**、**`export-image`**：校验旧形 JSON 后 **501**（**`o_video`** / OSS / 时间线仍待 **`jobs`** + 存储）；其余 **`/api/production/*`** 路径仍 ⏳ |
| `/api/project/*` | 项目、导演/视觉手册 | 🟡 | CRUD + `director_manual` 等已在 PG；**`GET`/`POST /api/v1/visual-manual`** 读 **`data/skills/art_skills/*`**（**POST** 与旧 **`POST /api/project/getVisualManual`** 动词一致，body 忽略；Markdown 槽位 + **`image`** 为 **`data/skills` 下相对路径**，无 OSS 署名；配图请求 **`GET /api/v1/skills/binary?path=`**）；Flutter **`fetchVisualManualV1`** / **`fetchVisualManualPostV1`** + 首页探针（GET） |
