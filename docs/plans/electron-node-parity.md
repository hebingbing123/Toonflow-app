...
| `/api/setting/vendorConfig/*` | 供应商与密钥 | 🟡 | 密钥不落 HTTP；**`GET /api/v1/settings/vendors/summary`** 提供嵌入 **`models_catalog`** 的 **无密钥** 供应商摘要（**`id`/`name`/`modelCount`/`modelKinds`**，**`source: static_catalog`**），对齐 **`getVendorList`** 的「列表」意图而非 **`inputValues`**；写路径 / **`modelTest`** 等仍 ⏳ |
