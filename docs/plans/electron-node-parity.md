...
| `/api/setting/skillManagement/*` | Skills 列表/读写 | 🟡 | Rust：**`GET /api/v1/skills*`** + **`PUT /api/v1/skills/content`**（仅覆盖**已存在**文件，对齐 **`saveSkillContent`**）；**新建文件**仍 ⏳ |
...
| 波次 | 目标 | 依赖 |
|------|------|------|
| **A（当前基线）** | 项目/剧本/分镜、jobs、usage、memory、models、skills 读 + **PUT …/skills/content**（覆盖已存在文件）、harness、billing webhook、me | 已有 |
