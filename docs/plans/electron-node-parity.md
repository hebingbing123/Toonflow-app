...
| `/api/setting/vendorConfig/*` | 供应商与密钥 | 🟡 | 密钥不落 HTTP；**`GET …/settings/vendors/summary`** 返回 **`static_catalog_with_user_config`**（静态目录 + **`app_user_profile.vendor_config`**）；**`POST …/settings/vendors/model-test`** 校验 + 配额 → 入队 **`settings.vendor.model_test`**（worker 暂 **`failed`**）；**`pg_contract`** **`settings_vendor_model_test_enqueue`**；**`POST …/vendors/{add,update,delete,enable,update-code,code-from-link}`** 均已落到 PG：新增 **`custom-*`** / **`linked-*`** vendor 元数据、更新展示名/选中模型/非敏感设置、启停、删除、保存 **`ts_code`** 或 **`code_link`**（不执行 TS、不服务端抓取外链）；OpenAPI 与首页探针已从旧 **501 stub** 改为真实 **200/404/503** 契约；**`pg_contract`** **`vendor_config_enable_update_roundtrip`** 现覆盖 enable/update/add/update-code/code-from-link/delete；供应商 live probe、密钥单独走 **`/credential`** |
