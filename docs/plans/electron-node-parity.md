...
| `/api/setting/memoryConfig/*` | 记忆配置 UI | 🟡 | **RAG / 摘要数值与 ONNX 路径**：**`GET`/`POST /api/v1/settings/memory-config`**（**camelCase**，默认同 **`initDB`** **`o_setting`**；进程内 **`RwLock`**，重启复位；**`delAllMemory`** 仍 ⏳）；与 **`/api/agents/*`** 会话记忆不同；OpenAPI **`getMemoryConfigV1`** / **`postMemoryConfigV1`**；Flutter 探针 |
