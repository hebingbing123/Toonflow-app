...
| `/api/migrate/migrateData` | 数据迁移 | 🔀 | **`toonflow-legacy-import`** + **`promote_legacy_from_staging()`**（含 **`o_novel`→`app_novel`**、**`o_assets`→`app_asset`**、**`o_scriptAssets`→`app_script_asset`**、**`o_artStyle`→`app_art_style`**（**`legacy_user_map` 最小 id** 定 **`owner_user_id`**）；返回 **`art_styles_upserted`** 等七列；非 HTTP 热路径） |
