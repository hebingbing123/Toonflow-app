...
| `/api/scriptAgent/*` | 剧本 Agent 计划数据 | 🟢 | **`POST /api/v1/script-agent/get-plan-data`**、**`set-plan-data`**、**`update-data`**：Postgres **`app_script_agent_plan`** + **`app_script`**（JWT + **`app_project.legacy_id`**）；legacy **`{ code, data, message }`** 包壳；Flutter 同上；首页 models 探针接受 **200** / **404** / **503**；🔀 实时流仍以 **Harness WS** 为主 |
