...
| 符号 | 行数 | 含义 |
|------|------|------|
| ✅ | **21** | 能力已对齐或明确替代（agents 记忆行、**`generalStatistics`**、**`general/getSingleProject+updateProject`**、**`project/*`**、**`script/*`**、**`quality/*`**、**`modelSelect`**（含旧 **`getTextModel`** / **`GET …/models/text-default`**）、**`getVersion`**、**`deleteAllData`**、**`clearData`**、**`production`**、**`scriptAgent`**、**`test`**、**`setting/dev`**、**`setting/about`**、**`setting/agentDeploy`**、**`setting/memoryConfig`**、**`setting/promptManage`**、**`setting/skillManagement`**、**`setting/vendorConfig`**、**`task`**） |
| 🔀 | **4** | 换设计、不逐路径复刻（**`login`**、**`migrate`**、**`openFolder`**、**`loginConfig`**） |
| 🟡 | **5** | 仍标「部分覆盖」：契约/竖切已有，备注里常见 **501**、**⏳**（出图、OSS、事件管线等）待产品里程碑 |
