...
| `/api/project/*` | 项目、导演/视觉手册 | 🟡 | CRUD + `director_manual` 等已在 PG；**`POST /api/v1/project/get-project`**（**`{}`**）→ **`{ data: [ProjectRow] }`**（对齐旧 **`getProject`** 列表）；**`POST …/project/delete-project`**（**`{ id }`**）等同 **`DELETE …/projects/legacy/{id}`**；**`GET`/`POST /api/v1/visual-manual`** 读 **`data/skills/art_skills/*`**（**POST** 与旧 **`POST /api/project/getVisualManual`** 动词一致，body 忽略；Markdown 槽位 + **`image`** 为 **`data/skills` 下相对路径**，无 OSS 署名；配图请求 **`GET /api/v1/skills/binary?path=`**）；Flutter **`fetchVisualManualV1`** / **`fetchVisualManualPostV1`** + 首页探针（GET） |
