...
| `/api/general/generalStatistics` | 多项目统计 | ✅ `GET /api/v1/projects/summary`（含 **`novel_count`**） | 单项目见 **`…/stats`**（**`novel_count`** + **`role_count`** 等） |
