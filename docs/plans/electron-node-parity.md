...
| `/api/setting/vendorConfig/*` | 供应商与密钥 | 🟡 | 密钥不落 HTTP；**`GET …/settings/vendors/summary`** 无密钥摘要；**`POST …/settings/vendors/model-test`** 校验 **`modelName`/`type`/`id`** 后 **501**（无在线探针）；写路径等仍 ⏳ |
