...
| `/api/cornerScape/getAllAssets` | 角落素材 | 🟡 | **`POST /api/v1/projects/legacy/{id}/assets/corner-scape`**（JWT）：父级资产（**`metadata.assetsId`** 缺失或 JSON **`null`**）、**`types`** 可选过滤、**role → scene → tool** 排序；**`history_images`** 来自 **`app_asset_image`**（**`state = '已完成'`**）；**`metadata`** 为父 **`app_asset`** 快照；**`POST …/assets/{aid}/images`** 写入 **`app_asset_image`**（默认 **`state=已完成`**）；Flutter **`createProjectAssetImage`**、项目对话框 **POST 首条资产图片**；**`pg_contract`** 经该 API 插入历史 |
