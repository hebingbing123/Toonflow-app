...
| 项 | 内容 |
|----|------|
| **目标** | **必做**：运营可按币种 / provider / 时间窗审计订阅与 webhook（API + 最小可用 UI 或受控导出）；能力建立在 `GET …/webhooks/billing/events` 等现有接口之上，缺口则 **必扩 query**。当前 Flutter「帮助 / 出站 Webhook」页已补最小可用 billing webhook 审计 UI（provider / informational / event_type / raw/provider event id 与 prefix / `event_created_*` + `created_*` 时间窗 / 分页）。 |
