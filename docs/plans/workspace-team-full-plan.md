...
- [ ] **W4.5** `app_generation_job`（及 payload）：是否挂 `workspace_id` 或由 `project_id` 派生 — **书面定稿** + 列表/取消/统计接口一致（进行中：先按 `project_id` 派生语义收口，`GET /api/v1/jobs/page`、`GET /api/v1/jobs` 以及 jobs 三个 summary 端点（kinds / kinds/summary / status/summary）在传 `project_id` 时改为 workspace 成员可见项目内任务；不传 `project_id` 仍保持 owner 视图）
