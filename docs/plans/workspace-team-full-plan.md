...
- [ ] **W4.6** `GET /api/v1/usage/summary`、memory、skills、quality 等：**定义**是否按 workspace 聚合或保持 user；**实现与 OpenAPI 一致**（进行中：`GET /api/v1/usage/summary` 已在响应与 OpenAPI 显式标注 `scope = user`；`GET /api/v1/skills/summary`、`GET /api/v1/agents/memory/cost-overview`、`POST /api/v1/agents/memory/query`、quality 聚合端点（`/quality/stats`、`/quality/stage-pass-rate`、`/quality/stage-grade-distribution`、`/quality/scope-insights`、`/quality/token-efficiency`、`/quality/token-efficiency/samples`）已补 `scope=user` 响应语义，统一为用户口径；其余 memory 聚合接口继续补齐）
