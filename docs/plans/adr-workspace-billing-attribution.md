...
Toonflow currently implements **user-scope billing**, where subscriptions, quotas, and usage tracking are attributed to individual users via `app_user_profile`. As we expand workspace capabilities (W8.2–W8.4), we must formally decide whether billing attribution should:
