...
- [ ] **Test rollback in isolated staging DB**
  ...
  createdb toonflow_staging_rollback_test
  
  # Apply migrations
  supabase db push --db-url postgresql://...
  
  # Apply rollback
  psql toonflow_staging_rollback_test < supabase/migrations/XXXXXX_rollback_workspace_billing.sql
  
  # Verify rollback successful
  psql toonflow_staging_rollback_test -c "\d app_workspace"
  ```
...
- [ ] **Enable dual-write in staging**
  ```bash
  export TOONFLOW_BILLING_DUAL_WRITE_ENABLED=true
...
- [ ] **Verify reconciliation metrics exist**
  ```bash
  curl http://staging-api.toonflow.com/metrics | grep billing_webhook_dual_write_mismatch_total
  ```
...
- [ ] **Verify reconciliation job runs**
  ...
  kubectl logs -l app=toonflow-backend --tail=100 | grep reconciliation
  ```
...
- [ ] **Create job in personal workspace**
  ```bash
  curl -X POST https://staging-api.toonflow.com/api/v1/jobs \
...
- [ ] **Enable workspace-scope quota**
  ```bash
  export TOONFLOW_BILLING_WORKSPACE_SCOPE_ENABLED=true
  # Restart staging backend
  ```
...
- [ ] **Verify quota denial metrics**
  ```bash
  curl http://staging-api.toonflow.com/metrics | grep quota_denied_total
...
- [ ] **Verify quota check logs**
  ```bash
  kubectl logs -l app=toonflow-backend --tail=100 | grep quota
...
- [ ] **Verify v1 response unchanged**
  ...
  curl -H "Authorization: Bearer $USER_TOKEN" \
    https://staging-api.toonflow.com/api/v1/me | jq '.'
  ```
...
- [ ] **Verify v1 OpenAPI schema**
  ```bash
  curl https://staging-api.toonflow.com/openapi.json | jq '.paths["/api/v1/me"].get.responses["200"]'
  ```
...
- [ ] **Verify v2 response structure**
  ```bash
  curl -H "Authorization: Bearer $USER_TOKEN" \
    'https://staging-api.toonflow.com/api/v1/me?v=2' | jq '.'
  ```
...
- [ ] **Verify v2 OpenAPI schema**
  ```bash
  curl https://staging-api.toonflow.com/openapi.json | jq '.paths["/api/v1/me"].get.parameters'
...
- [ ] **Test v2 request**
  ```bash
  curl -H "Authorization: Bearer $USER_TOKEN" \
    'https://staging-api.toonflow.com/api/v1/me?v=2'
  # Should return 400 or fall back to v1
  ```

- [ ] **Re-enable v2**
  ```bash
  export TOONFLOW_ME_V2_ENABLED=true
  # Restart staging backend
  ```
...
- [ ] **Review reconciliation logs**
  ```bash
  kubectl logs -l app=toonflow-backend --since=24h | grep reconciliation
  ```
...
- [ ] **Monitor database write latency**
  ...
  curl http://staging-api.toonflow.com/metrics | grep http_request_duration_seconds
  ```

- [ ] **Monitor quota check latency**
  ```bash
  # Check p99 latency for quota checks
  curl http://staging-api.toonflow.com/metrics | grep quota_check_duration_seconds
  ```
...
- [ ] **`yarn refactor:check` passes**
  ```bash
  cd /Users/clive/Documents/source/cousor/Toonflow-app
  yarn refactor:check
  ```
