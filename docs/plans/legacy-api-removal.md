...
| **URL 中含 `/legacy/{整型 id}`** | 如 `GET /api/v1/scripts/legacy/1`（剧本/分镜等仍常见） | 中：按域竖切为 UUID 项目段或资源 UUID |
...
| `backend/src/scripting/scripts.rs` | **主路径**：`POST …/projects/{project_id}/scripts`、`POST …/projects/{project_id}/scripts/batch-add`，`GET`/`PATCH`/`DELETE …/projects/{project_id}/scripts/{script_legacy_id}`。**已删除** `…/projects/legacy/{id}/scripts`。**仍保留**：`…/scripts/legacy/{id}`（按剧本 **`legacy_id`**）；旧 `POST …/scripts/batch-add` 已移除 |
