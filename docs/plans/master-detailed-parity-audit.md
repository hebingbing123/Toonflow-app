...
- 当前分支 `backend/src/**/*`
- 当前分支 `frontend/lib/**/*`
- 合并 OpenAPI：`GET /api/v1/openapi.yaml` 或 `backend/` 下 `cargo run --bin export-openapi`（源：`shell.rs` + `embedded/legacy_component_schemas.json` + 已提交 utoipa stubs / 域 `OpenApi`，**不含**后端 `openapi_paths_index.yaml`）
