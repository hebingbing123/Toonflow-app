...
-- Runtime gates: TOONFLOW_NUMERIC_ID_LEGACY_READ / TOONFLOW_NUMERIC_ID_LEGACY_WRITE (default true).
-- Target sunset for numeric-only clients: 2026-11-01 (operational).

COMMENT ON COLUMN public.app_project.numeric_id IS
  'Legacy per-owner integer project id (Electron/SQLite era). UUID app_project.id is canonical. Removal window: keep through ~2026-11; drop only after promote_import_snapshots UUID path and jobs UUID filter. Env: TOONFLOW_NUMERIC_ID_LEGACY_READ/WRITE.';
