-- Extend promote: upsert **`app_novel`** from **`o_novel`** snapshots (after **`app_novel`** table exists).
...
BEGIN
  ...
  RETURN QUERY
  SELECT p_count, s_count, b_count, n_count;
END;
$$;
...
COMMENT ON FUNCTION public.promote_legacy_from_staging IS 'Upsert app_project, app_script, app_storyboard, app_novel from legacy_staging; service_role only';
