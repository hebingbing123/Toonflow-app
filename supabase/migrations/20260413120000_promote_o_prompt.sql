-- Extend **`promote_legacy_from_staging`**: upsert **`app_user_prompt`** from **`o_prompt`** snapshots.
-- **`owner_user_id`** = **`legacy_user_map`** row with smallest **`legacy_user_id`** (same single-user rule as **`o_artStyle`**).
-- Only known **`type`** values and non-empty **`data`** are imported; **`legacy_id`** comes from SQLite **`id`**.
...
BEGIN
  ...
  RETURN QUERY
  SELECT p_count, s_count, b_count, n_count, a_count, sa_count, as_count, pr_count;
END;
$$;
...
COMMENT ON FUNCTION public.promote_legacy_from_staging IS 'Upsert app_project, app_script, app_storyboard, app_novel, app_asset, app_script_asset, app_art_style, app_user_prompt from legacy_staging; service_role only';
