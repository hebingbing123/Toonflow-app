-- Extend promote: upsert **`app_art_style`** from **`o_artStyle`** snapshots (after **`app_art_style`** exists). **`owner_user_id`** = smallest **`legacy_user_map.legacy_user_id`** row (single-user SQLite import); if **`legacy_user_map`** is empty, no art styles are inserted.
...
BEGIN
  ...
  RETURN QUERY
  SELECT p_count, s_count, b_count, n_count, a_count, sa_count, as_count;
END;
$$;
...
COMMENT ON FUNCTION public.promote_legacy_from_staging IS 'Upsert app_project, app_script, app_storyboard, app_novel, app_asset, app_script_asset, app_art_style from legacy_staging; service_role only';
